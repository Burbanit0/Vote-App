"""
Shared async-DB test harness for api_v2 (Phase 4.5.b.2).
...
Only tests that request these fixtures get them — non-DB api_v2 suites are
untouched.
"""
import asyncio
...
from api_v2.db import Base, GalleryScenario, User
from api_v2.db import session as db_session
...
def db(tmp_path, monkeypatch):
    """File-backed aiosqlite engine wired into api_v2.db.session for the test,
...
def make_token(user_id: int, secret: str = JWT_SECRET) -> str:
    """Mint a Bearer token in the shape api_v2.core.auth expects (sub=str(id))."""
