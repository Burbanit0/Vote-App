...
class ElectionService:
    ...
    def get_all_elections(page, per_page, search, status, sort_by, sort_dir):
        ...
        query = db.session.query(Election, User.first_name, User.last_name).join(User, Election.created_by == User.id)
        ...
        if sort_by == 'name':
            query = query.order_by(Election.name.desc() if sort_dir == 'desc' else Election.name.asc())
        elif sort_by == 'date':
            query = query.order_by(Election.start_date.desc() if sort_dir == 'desc' else Election.start_date.asc())

        paginated_elections = query.paginate(page=page, per_page=per_page, error_out=False)
