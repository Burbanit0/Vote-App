...
def get_results():
    ...
    return jsonify([{'id': r.id, 'candidate_id': r.candidate_id,
                    'vote_count': r.vote_count, 'vote_type': r.vote_type}
                    for r in results])


@bp.route('/majority', methods=['GET'])
def get_majority():
    total_sum = db.session.query(db.func.sum(Result.vote_count)).filter(
        Result.vote_type == 'single').scalar()
    ...
    ).filter(Result.vote_type == "single").all()
    ...
    for result, first_name, last_name in results:
        result_info = {
            ...
            'result': (result.vote_count / total_sum * 100) if total_sum != 0
            else 0
        }
