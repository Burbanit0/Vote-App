...
from app.utils.simul import simulate_voters, simulate_score_voters, \
    simulate_ranked_voters
from app.utils.simulation_ranked_utils import \
    get_condorcet_winner, \
    get_two_round_winner, get_borda_winner, get_plurality_winner, \
    get_approval_winner, get_irv_winner, get_coombs_winner, \
    get_score_winner, get_kemeny_young_winner, get_bucklin_winner, \
    get_minimax_winner, get_schulze_winner
from app.simulation.population_simulation import \
    assign_voters_to_candidates
from app.utils.simulation_voting_utils import calculate_utility, \
    create_voter, create_candidate
from app.utils.simulation_score_utils import get_mean_median_hybrid_winner, \
    get_median_voting_winner, get_score_distribution_analysis, \
...
def simulate_votes_route():
    ...
    if "votes" in simulation_type:
        voters, votes, tally = simulate_voters(population_size, candidates,
                                               demographics,
                                               influence_weights,
                                               turnout_rate)
    ...
    if "scores" in simulation_type:
        ...
        score_distribution_analysis = get_score_distribution_analysis(
            all_scores)
    ...
    if "votes" in simulation_type:
        response.update({
            "votes": [{'voter_id': voter['id'], 'preference':
                       voter['preference']} for voter in voters
                      if voter['turnout']],
            "tally": tally,
            "voters_sample": [{k: v for k, v in voter.items() if k != 'scores'
                               and k != 'ranking'}
                              for voter in voters]
            })
    ...
    if "ranked" in simulation_type:
        response.update({
            "rankings": [{'voter_id': voter['id'], 'ranking': voter['ranking']}
    ...
    if "scores" in simulation_type:
        response.update({
            "all_scores": [{'voter_id': voter['id'], 'scores': voter['scores']}
                           for voter in voters_n if voter['turnout']],
            "avg_scores": avg_scores,
            "voters_sample": [{k: v for k, v in voter.items()
    ...
    try:
        ...
        for voter in voters:
            for candidate in candidates:
                utility_results.append(calculate_utility(voter, candidate,
                                                         issues))
...
def get_voter_segments():
    ...
    try:
        ...
        segment_types = data.get('segments', [
            "young_female", "old_male", "high_edu", "low_income", "urban",
            "rural"
        ])
        ...
        segment_definitions = {
            ...
            "urban": {
                "test": lambda v: v["region"] == "urban",
                "label": "Urbains"
            },
            "rural": {
                "test": lambda v: v["region"] == "rural",
                "label": "Ruraux"
            }
        }
        ...
        # Filter segments
        segment_definitions = {
            k: v for k, v in segment_definitions.items()
            if k in segment_types
        }
        ...
        for seg_key, seg_def in segment_definitions.items():
            ...
            for voter in segment_voters:
                voter_utilities = [
                    r for r in utility_results
                    if r["voter_id"] == voter["id"]
                ]
                if voter_utilities:
            ...
            if candidate_votes:
                ...
                top_candidate = next(
                    (c for c in candidates if c["id"] == top_candidate_id),
                    None
                )
                top_utility = next(
                    (u["utility"] for u in segment_utilities if u["candidate_id"] == top_candidate_id),
                    0
                )
            else:
                top_candidate = None
