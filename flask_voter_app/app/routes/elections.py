...
def get_all_elections():
    ...
    result = ElectionService.get_all_elections(
        page, per_page, search, status, sort_by, sort_dir
    )
...
def create_election():
    ...
    result = ElectionService.create_election(
        name, description, start_date, end_date, current_user_id
    )
...
def get_election(election_id):
    ...
    election = (
        db.session.query(Election, User.first_name, User.last_name)
        .join(User, Election.created_by == User.id)
    ...
    formatted_election = {
        ...
        "start_date": (
            election[0].start_date.isoformat() if election[0].start_date else None
        ),
        "end_date": election[0].end_date.isoformat() if election[0].end_date else None,
...
def get_election_participants(election_id):
    ...
    participants = (
        db.session.query(User, user_election_roles.c.role)
        .join(
            user_election_roles,
    ...
    if not existing_participation:
        ...
        db.session.execute(
            user_election_roles.insert().values(
                user_id=user_id, election_id=election_id, role=ElectionRole.VOTER
            )
        )

        # Update participation metrics
        user.elections_participated += 1
        ParticipationService.handle_election_participation(user_id, election_id, role)
        db.session.commit()
    else:
        return (
            jsonify({"message": "User is already participating in this election"}),
            400,
        )
...
def register_as_candidate(election_id):
    ...
    if existing_candidate_role:
        return jsonify({"message": "User is already a candidate in this election"}), 400
...
@election_bp.route("/<int:election_id>/participation", methods=["GET"])
@jwt_required()
def check_election_participation(election_id):
    ...
    if is_participant:
        role = (
            db.session.query(user_election_roles.c.role)
            .filter(
                user_election_roles.c.user_id == current_user_id,
                user_election_roles.c.election_id == election_id,
            )
    ...
    return jsonify({"is_participant": is_participant, "role": user_role})
...
def cancel_participation(election_id):
    ...
    if not election or election.start_date.replace(tzinfo=timezone.utc) <= datetime.now(
        timezone.utc
    ):
        return (
            jsonify(
                {"message": "Cannot cancel participation after " "election has started"}
            ),
            400,
        )
...
def get_election_results(election_id):
    ...
    # Get overall results
    overall_results = (
        db.session.query(User.username, func.count(Vote.id).label("vote_count"))
        .join(Vote, Vote.candidate_id == User.id)
    ...
    results["overall"] = [
        {"candidate": candidate, "votes": votes} for candidate, votes in overall_results
    ]
