...
export const registerUser = async (username: string, password: string, role: 'User' | 'Admin', first_name: string, last_name: string) => {
  try {
...
export const checkPermissions = async (): Promise<UserPermissions> => {
  const storedUserJSON = localStorage.getItem('user');
    ...
    try {
      const response = await axios.get(
          `${API_BASE_URL}/api/auth/users/me/permissions`,
          {
              headers: {
                Authorization: `Bearer ${token}`,
              }
...
export const getParticipation = async() : Promise<ParticipationData> => {
  const storedUserJSON = localStorage.getItem('user');
    ...
    try {
      const response = await axios.get(
          `${API_BASE_URL}/api/auth/users/me/participation`,
          {
              headers: {
                Authorization: `Bearer ${token}`,
              }
