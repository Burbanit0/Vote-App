...
interface DistrictBounds { x_min: number; x_max: number; y_min: number; y_max: number }
interface DistrictDef    { id: number; bounds: DistrictBounds }
...
interface VoterSnap { id: number; x: number; y: number; preferred: string }
...
const DIST_COLORS = [
  '#005CAB44', '#C8590A44', '#007A3344', '#6c757d44',
  '#9b59b644', '#e67e2244', '#2A9D8F44', '#E76F5144',
  '#f0c04044', '#b71c1c44',
];
const DIST_STROKE = [
  '#005CAB', '#C8590A', '#007A33', '#6c757d',
  '#9b59b6', '#e67e22', '#2A9D8F', '#E76F51',
  '#f0c040', '#b71c1c',
];
...
function domainToSvgX(x: number) { return (x + 1) / 2 * SVG_W; }
function domainToSvgY(y: number) { return (1 - y) / 2 * SVG_H; }  // flip y
...
const Hémicycle: React.FC<{
  ...
  const W = 200; const H = 120; const cx = W / 2; const cy = H - 8;
  const rI = 35; const rO = 90;
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxWidth: 220 }}>
        {segs.map(s => (
          <path key={s.name} d={path(rI, rO, s.a1, s.a2)}
            fill={candColor(s.name, names)} stroke="#fff" strokeWidth={1}>
...
const GerrymanderMap: React.FC = () => {
  ...
  const autoRedistribute = () => {
    setGrid(Array.from({ length: GRID }, (_, row) =>
  ...
  const winnerPct = data
    ...
    : 0;
  const winnerVote = data
    ? Math.round((data.national_vote_share[data.winner] ?? 0) * 100)
    : 0;
  ...
  return (
    <div>
      ...
      <Row className="g-2 mb-3 align-items-end">
        <Col xs={12} sm={3}>
          <div className="small mb-1">{t('gerrymander.numDistricts')}: <strong>{numDist}</strong></div>
          <input type="range" className="form-range" min={2} max={10} step={1} value={numDist}
            onChange={e => {
        ...
        <Col xs={12} sm={3} className="d-flex gap-1 flex-wrap">
      ...
      <Row className="g-3">
        ...
        <Col xs={12} lg={6}>
          <div className="small text-muted mb-1">{t('gerrymander.gridHint')}</div>
          <svg
            ref={svgRef}
            ...
            {Array.from({ length: numDist }, (_, distId) => {
              ...
              return (
                <text key={distId}
                  ...
                  style={{ pointerEvents: 'none' }}
                >
                  {distId + 1}{distResult?.winner ? ` (${distResult.winner[0]})` : ''}
                </text>
              );
            })}
          </svg>
          ...
          <div className="d-flex flex-wrap gap-2 mt-1" style={{ fontSize: '0.7rem' }}>
            {Array.from({ length: numDist }, (_, i) => (
              <span key={i} className="d-flex align-items-center gap-1">
                <span style={{ width: 12, height: 12, background: DIST_COLORS[i], border: `2px solid ${DIST_STROKE[i]}`, display: 'inline-block', borderRadius: 2 }} />
        ...
        {data && (
          <Col xs={12} lg={6}>
            {/* Pedagogical message */}
            <Alert variant={isGerry ? 'warning' : 'success'} className="py-2 mb-3"
              style={{ fontSize: '0.82rem' }} data-testid="gerrymander-alert">
              {isGerry
            ...
            <div className="mb-3">
              <div className="d-flex justify-content-between mb-1" style={{ fontSize: '0.75rem' }}>
                ...
                <strong>{t('gerrymander.index')}: {Math.round(data.gerrymander_index * 100)}%</strong>
              ...
              <div style={{ height: 12, background: '#e9ecef', borderRadius: 6, overflow: 'hidden' }}>
                <div style={{
                  ...
                  height:     '100%',
                  background: data.gerrymander_index > 0.5 ? '#dc3545' : data.gerrymander_index > 0.2 ? '#f0c040' : '#007A33',
                  borderRadius: 6,
            ...
            <Row className="g-2 mb-3">
              <Col xs={6}>
                <Hémicycle seats={data.parliament_gerrymander}
                  names={data.candidates} total={data.num_seats}
                  label={t('gerrymander.labelGerry')} />
              </Col>
              <Col xs={6}>
                <Hémicycle seats={data.parliament_proportional}
                  names={data.candidates} total={data.num_seats}
            ...
            <div className="d-flex flex-wrap gap-2">
              ...
              {data.candidates.map(n => {
                ...
                return (
                  <Badge key={n} style={{ background: candColor(n, data.candidates), fontSize: '0.7rem' }}>
                    {n}: {seatsG} {diff !== 0 && <span>({diff > 0 ? '+' : ''}{diff})</span>}
                  </Badge>
                );
              })}
            </div>
