...
const VoterCompositionBars: React.FC<CompositionProps> = ({ elections, t }) => {
  ...
  return (
    <div data-testid="composition-bars" className="mb-3">
      <div className="fw-semibold mb-1" style={{ fontSize: '0.82rem' }}>{t('fatigue.compositionTitle')}</div>
      <div className="d-flex gap-1 align-items-end">
        {elections.map((e) => {
          const partPct = Math.round(e.voter_profile.partisan_pct * 100);
          const casuals = 100 - partPct;
          return (
            <div key={e.election_n} className="d-flex flex-column align-items-center"
              style={{ flex: 1, minWidth: 24 }}>
              <div style={{ width: '100%', height: COMP_H, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                <div style={{ height: `${Math.round(e.turnout * COMP_H)}px`, background: '#dee2e6', borderRadius: '3px 3px 0 0', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '0.6rem', color: '#6c757d' }}>{Math.round(e.turnout * 100)}%</div>
            </div>
          );
        })}
      </div>
      <div className="d-flex gap-3 mt-1" style={{ fontSize: '0.7rem', color: '#6c757d' }}>
        <span><span style={{ background: '#dc3545', display: 'inline-block', width: 10, height: 10, marginRight: 3, borderRadius: 2 }} />
          {t('fatigue.partisan')}</span>
        <span><span style={{ background: '#adb5bd', display: 'inline-block', width: 10, height: 10, marginRight: 3, borderRadius: 2 }} />
          {t('fatigue.casual')}</span>
      </div>
    </div>
  );
};
...
const ElectoralFatiguePanel: React.FC = () => {
  ...
  const schedule = useCallback((fr: number, ep: number, ne: number) => {
  ...
  // ── Chart data ──────────────────────────────────────────────────────────
  const lineData = data?.elections.map((e) => ({
  ...
  const areaData = data?.elections.map((e) => ({
    election: e.election_n,
    ...Object.fromEntries(
  ...
  return (
    <div>
      ...
      {!data && !loading && !error && (
        <Alert variant="info" role="alert">{t('fatigue.prompt')}</Alert>
      )}
      ...
      {data && (
        <>
          ...
          <div className="mb-4" data-testid="turnout-chart">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={lineData} margin={{ top: 4, right: 32, bottom: 4, left: 0 }}>
                ...
                <XAxis dataKey="election" label={{ value: t('fatigue.electionN'), position: 'insideBottom', offset: -2, fontSize: 10 }} />
                ...
                <ReferenceLine yAxisId="right" y={data.full_mean_ideology}
                  stroke="#6c757d" strokeDasharray="4 2"
                  label={{ value: 'μ élect.', fill: '#6c757d', fontSize: 9 }} />
                <Line yAxisId="left" type="monotone" dataKey="turnout"
                  name={t('fatigue.turnout')} stroke="#005CAB" strokeWidth={2} dot />
                <Line yAxisId="right" type="monotone" dataKey="ideology"
                  name={t('fatigue.ideology')} stroke="#C8590A" strokeWidth={2} dot strokeDasharray="4 2" />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          {/* Winner drift */}
          <div className="d-flex flex-wrap gap-1 mb-3" data-testid="winner-drift-badges">
            {data.elections.map((e, i) => (
              <Badge
                key={i}
                style={{ background: candColor(e.winner ?? '', candidateNames), fontSize: '0.7rem' }}
              >
