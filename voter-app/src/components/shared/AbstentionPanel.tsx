    ...
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <text x={PAD + 2} y={PAD + 12} fontSize={9} fill="#6c757d">
          {t('abstention.round')} {round} — {Math.round((1 - abstained / Math.max(total, 1)) * 100)}
          % {t('abstention.turnout')}
        </text>
      </svg>
...
const AbstentionPanel: React.FC = () => {
  ...
  const chartData =
    data?.rounds.map((r) => ({
      ...
      ...Object.fromEntries(
        candidateNames.map((n) => [n, Math.round((data.turnout_by_camp[n] ?? 0) * 100)])
      ),
    })) ?? [];
        ...
        <Col xs={12} sm={4}>
          ...
          <Range
            ...
            onChange={(e) => {
              ...
              setDemob(v);
              handleChange(v, influence, numRounds);
            }}
        ...
        <Col xs={12} sm={4}>
          ...
          <Range
            ...
            onChange={(e) => {
              const v = Number(e.target.value);
              setInfluence(v);
              handleChange(demob, v, numRounds);
            }}
        ...
        <Col xs={12} sm={2}>
          ...
          <Range
            ...
            onChange={(e) => {
              const v = Number(e.target.value);
              setNumRounds(v);
              handleChange(demob, influence, v);
            }}
          />
        </Col>
        <Col xs={12} sm={2}>
          <Button
            variant="primary"
            className="w-full"
            onClick={() => run(demob, influence, numRounds)}
            disabled={loading}
          >
          ...
          >
            {differs ? (
              <span style={{ color: '#dc3545', fontWeight: 600 }}>
                ...
                {' — '}
                <span className="text-muted-foreground font-normal" style={{ fontSize: '0.85rem' }}>
          ...
          <Row className="g-3">
            ...
            <Col xs={12} lg={6}>
              <div className="border border-border rounded p-2">
                ...
                {/* Legend */}
                <div className="flex flex-wrap gap-2 mt-1" style={{ fontSize: '0.72rem' }}>
                    ...
                    <span key={n} className="flex items-center gap-1">
                      <span
                        style={{
                          ...
                          height: 10,
                          borderRadius: '50%',
                          background: candColor(n, candidateNames),
            ...
            <Col xs={12} lg={6}>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 8, right: 16, bottom: 4, left: -8 }}>
                    ...
                    <Line
                      type="monotone"
                      dataKey="turnout"
                      name={t('abstention.globalTurnout')}
                      stroke="#212529"
                      strokeWidth={2}
                      dot={{ r: 3 }}
                    ...
                    {showCampTurnout &&
                      candidateNames.map((n) => (
                        <Line
                          ...
                          type="monotone"
                          dataKey={n}
                          name={`${n} ${t('abstention.campSuffix')}`}
                          stroke={candColor(n, candidateNames)}
                          strokeWidth={1.5}
                          strokeDasharray="4 2"
                          dot={false}
                          isAnimationActive={false}
              ...
              {showCampTurnout && (
                <div className="mt-2" style={{ fontSize: '0.78rem' }}>
                  <div className="font-semibold mb-1">{t('abstention.finalTurnoutByCamp')}</div>
                  <div className="flex flex-wrap gap-2">
                    {candidateNames.map((n) => {
                      ...
                      return (
                        <Badge
                          key={n}
                          style={{ background: candColor(n, candidateNames), fontSize: '0.72rem' }}
