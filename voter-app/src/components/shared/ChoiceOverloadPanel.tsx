...
const ChoiceOverloadPanel: React.FC = () => {
  ...
  const [hNotoriety,  setHNotoriety]  = useState(0.20);
  ...
  // ── Line chart data ─────────────────────────────────────────────────────
  const lineData = data?.results_by_n.map((r) => ({
    n:        r.num_candidates,
  ...
  const totalH = data
    ? data.heuristic_weights.notoriety + data.heuristic_weights.primacy + data.heuristic_weights.partisan
    : hNotoriety + hPrimacy + hPartisan;

  const barData = data?.results_by_n.map((r) => {
  ...
  const overThreshold = data?.results_by_n.filter((r) => r.num_candidates > data.overload_threshold) ?? [];
  ...
  return (
    <div>
      ...
      <Row className="g-2 mb-3 align-items-end">
        <Col xs={12} md={3}>
          ...
          <Form.Range
            ...
            min={2} max={10} step={1}
            value={threshold}
            onChange={(e) => { setThreshold(Number(e.target.value)); schedule(hNotoriety, hPrimacy, hPartisan, Number(e.target.value)); }}
          />
        </Col>
        <Col xs={12} md={3}>
          <Form.Label className="small mb-0">
          ...
          <Form.Range
            ...
            value={hNotoriety}
            onChange={(e) => { setHNotoriety(Number(e.target.value)); schedule(Number(e.target.value), hPrimacy, hPartisan, threshold); }}
          />
        </Col>
        <Col xs={12} md={3}>
          <Form.Label className="small mb-0">
          ...
          <Form.Range
            ...
            min={0} max={0.5} step={0.05}
            value={hPrimacy}
            onChange={(e) => { setHPrimacy(Number(e.target.value)); schedule(hNotoriety, Number(e.target.value), hPartisan, threshold); }}
          />
        </Col>
        <Col xs={12} md={3}>
          <Form.Label className="small mb-0">
          ...
          <Form.Range
            ...
            min={0} max={0.5} step={0.05}
            value={hPartisan}
            onChange={(e) => { setHPartisan(Number(e.target.value)); schedule(hNotoriety, hPrimacy, Number(e.target.value), threshold); }}
          />
        </Col>
      </Row>
      ...
      {!data && !loading && !error && (
        <Alert variant="info" role="alert">{t('overload.prompt')}</Alert>
      )}
          ...
          <div className="mb-4" data-testid="overload-line-chart">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={lineData} margin={{ top: 4, right: 40, bottom: 4, left: 0 }}>
                ...
                <XAxis dataKey="n" label={{ value: t('overload.nCandidates'), position: 'insideBottom', offset: -2, fontSize: 10 }} />
                ...
                <ReferenceLine yAxisId="left" x={data.overload_threshold}
                  stroke="#dc3545" strokeDasharray="4 2"
                  label={{ value: t('overload.thresholdLine'), fill: '#dc3545', fontSize: 9 }} />
                <Line yAxisId="left" type="monotone" dataKey="regret"
                  name={t('overload.regret')} stroke="#dc3545" strokeWidth={2} dot />
                <Line yAxisId="right" type="monotone" dataKey="heuristic"
                  name={t('overload.heuristicVoters')} stroke="#fd7e14" strokeWidth={2} dot strokeDasharray="4 2" />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="mb-4" data-testid="heuristic-bar-chart">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={barData} margin={{ top: 4, right: 16, bottom: 4, left: 0 }}>
                ...
                <Bar dataKey="notoriety" name={t('overload.notoriety')} stackId="a" fill="#fd7e14" />
          ...
          <Table size="sm" hover className="mb-3" data-testid="method-table">
            ...
            <tbody>
              {data.results_by_n.map((r) => (
                <tr key={r.num_candidates}
                  style={{ background: r.num_candidates > data.overload_threshold ? '#fff5f5' : undefined }}>
                  <td style={{ fontSize: '0.78rem', fontWeight: 600 }}>{r.num_candidates}</td>
                  <td style={{ fontSize: '0.78rem', color: r.mean_voter_regret > 0.05 ? '#dc3545' : '#6c757d' }}>
                    {Math.round(r.mean_voter_regret * 1000) / 10}
                  </td>
                  <td style={{ fontSize: '0.78rem' }}>{Math.round(r.heuristic_voters * 100)}%</td>
                  {methods.map((m) => (
                    <td key={m} style={{ fontSize: '0.78rem' }}>{r.winner_by_method[m] ?? '—'}</td>
                  ))}
                </tr>
              ))}
            </tbody>
