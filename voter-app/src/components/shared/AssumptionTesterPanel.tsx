...
const VARIANCE_THRESHOLDS = { low: 0.15, medium: 0.4 };
  ...
  return (
    <div style={{ marginBottom: 6 }}>
      ...
      <div style={{ height: 6, background: '#dee2e6', borderRadius: 3, overflow: 'hidden' }}>
        <div
          style={{ width: `${pct}%`, height: '100%', background: color, transition: 'width 0.4s' }}
        />
      </div>
    </div>
  );
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <span className="font-semibold">{t(`assumptions.name_${id}`)}</span>
            {isMostFragile && (
          ...
          <div className="text-muted-foreground" style={{ fontSize: '0.7rem', marginTop: 2 }}>
                      ...
                      <div style={{ flex: 1, height: 8, background: '#dee2e6', borderRadius: 3 }}>
                        <div
                          style={{
                            ...
                            height: '100%',
                            background: ASSUMPTION_COLORS[id] ?? '#0d6efd',
                            borderRadius: 3,
                          }}
                        />
                      </div>
        ...
        <>
          ...
          >
            <div className="flex gap-3 flex-wrap items-center">
              <div>
                <span className="text-muted-foreground">{t('assumptions.baselineWinner')}:</span>{' '}
                <Badge variant="primary">{data.baseline_result.winner}</Badge>
              </div>
              <Badge variant={data.robust_result ? 'success' : 'danger'} data-testid="robust-badge">
                {data.robust_result ? t('assumptions.robust') : t('assumptions.fragile')}
              </Badge>
              {data.most_fragile_assumption && (
                <span>
                  <span className="text-muted-foreground">
                    {t('assumptions.mostFragileLabel')}:
                  </span>{' '}
          ...
          <div className="mt-3" data-testid="variance-chart">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={fragilityData} margin={{ top: 5, right: 20, bottom: 30, left: 0 }}>
          ...
          >
            <div className="font-semibold mb-2">{t('assumptions.referenceTitle')}</div>
              ...
              <tbody>
                {ALL_ASSUMPTIONS.map((a) => (
                  <tr key={a}>
                    ...
                    <td className="text-muted-foreground">{t(`assumptions.violation_${a}`)}</td>
