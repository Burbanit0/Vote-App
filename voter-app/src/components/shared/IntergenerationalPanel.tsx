...
const DEFAULT_DECISIONS: Decision[] = [
  {
    name: 'Retraite par répartition',
    cost_present: -0.2,
    benefit_future: -0.4,
    time_horizon_years: 30,
  },
  {
    name: 'Investissement éducation',
    cost_present: -0.3,
    benefit_future: 0.8,
  ...
  {
    name: 'Isolation des bâtiments',
    cost_present: -0.4,
    benefit_future: 0.7,
    time_horizon_years: 15,
  },
  {
    name: 'Fonds souverain climatique',
    cost_present: -0.3,
    benefit_future: 0.9,
...
const MiniHemicycle: React.FC<HemicycleProps> = ({ results, mechanism, color, label }) => {
  ...
  const slices = results.map((dr, i) => {
    ...
    const x2o = cx + R * Math.cos(aEnd),
      y2o = cy + R * Math.sin(aEnd);
    const x1i = cx + r * Math.cos(angle),
      y1i = cy + r * Math.sin(angle);
    const x2i = cx + r * Math.cos(aEnd),
      y2i = cy + r * Math.sin(aEnd);
    ...
    return (
      <path key={i} d={d} fill={adopted ? color : '#dee2e6'} stroke="#fff" strokeWidth={1}>
        <title>
          {dr.decision_name}: {adopted ? '✓ adopté' : '✗ rejeté'}
        </title>
      </path>
    );
  });
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <svg width={W} height={H} data-testid={`hemicycle-${mechanism}`}>
        {slices}
      </svg>
      <div style={{ fontSize: '0.7rem', fontWeight: 600, color }}>{label}</div>
...
const IntergenerationalPanel: React.FC = () => {
  ...
  const data: IntergenerationalData | null =
    (sim.data as IntergenerationalData | undefined) ?? null;
  ...
  const removeDecision = (i: number) => setDecisions((prev) => prev.filter((_, j) => j !== i));
  ...
  return (
    <div>
      ...
      <Alert
        variant="info"
        className="py-2 mb-3"
        data-testid="key-message"
        style={{ fontSize: '0.78rem', borderLeft: '4px solid #0d6efd' }}
      >
        <strong>{t('intergen.keyMessageTitle')}</strong> {t('intergen.keyMessage')}
      </Alert>
          ...
          <Row key={i} className="g-1 mb-1 items-center">
            <Col xs={12} md={3}>
              <Control
                size="sm"
                value={d.name}
                placeholder={t('intergen.decisionName')}
                data-testid={`decision-name-${i}`}
                onChange={(e) => updateDecision(i, 'name', e.target.value)}
              />
            </Col>
            <Col xs={4} md={2}>
              <label className="mb-1 inline-block text-sm mb-0" style={{ fontSize: '0.65rem' }}>
      ...
      <Row className="g-2 mb-3 items-end">
        <Col xs={6} md={2}>
          <label className="mb-1 inline-block text-sm mb-0">{t('intergen.voters')}</label>
          <Control
            type="number"
            size="sm"
            min={20}
        ...
        <Col xs={12} md={5}>
          <label className="mb-1 inline-block text-sm mb-0">
            {t('intergen.ageDist')} — {t('intergen.young')}: {ageDist[0]}% /{t('intergen.adult')}:{' '}
            {ageDist[1]}% / {t('intergen.senior')}: {ageDist[2]}%
          </label>
        ...
        <Col xs={6} md={1}>
          <label className="mb-1 inline-block text-sm mb-0">{t('intergen.seed')}</label>
          <Control
            type="number"
            size="sm"
            value={seed}
      ...
      {data && (
        <>
            ...
            <div data-testid="welfare-view">
              ...
              <ResponsiveContainer width="100%" height={240}>
                ...
                >
                  ...
                  <Bar
                    dataKey="none_future"
                    name={`${mechLabel('none')} — futur`}
                    fill={MECH_COLORS.none}
                    opacity={0.6}
                    stackId="a"
                  />
                  <Bar
                    dataKey="veto_future"
                    name={`${mechLabel('veto')} — futur`}
                    fill={MECH_COLORS.veto}
                    opacity={0.9}
                    stackId="b"
                  />
                  <Bar
                    dataKey="age_weighted_future"
                    name={`${mechLabel('age_weighted')} — futur`}
                    fill={MECH_COLORS.age_weighted}
                    opacity={0.9}
                    stackId="c"
                  />
                </BarChart>
              ...
              >
                ...
                <tbody>
                  {MECHANISMS.map((m) => {
                    ...
                    const isBest =
                      m ===
                      Object.keys(data.mechanism_comparison).reduce(
                        (best, k) =>
                          data.mechanism_comparison[k].avg_welfare_50y >
                          data.mechanism_comparison[best].avg_welfare_50y
                            ? k
                            : best,
                        'none'
                      );
                    return (
                      ...
                      >
                        ...
                        <td>
                          <span
                            style={{ color: s.future_welfare_gain >= 0 ? '#198754' : '#dc3545' }}
                          >
                            {s.future_welfare_gain >= 0 ? '+' : ''}
                            {Math.round(s.future_welfare_gain * 100)}%
                          </span>
                        </td>
          ...
          <div className="mt-3" data-testid="decisions-breakdown">
            ...
            <div className="flex flex-wrap gap-2">
              {data.decisions_results.map((dr, i) => (
                ...
                >
                  <div className="font-semibold mb-1" style={{ fontSize: '0.72rem' }}>
                    {dr.decision_name}
                  </div>
                  {MECHANISMS.map((m) => {
          ...
          >
            <div className="font-semibold mb-2">{t('intergen.examplesTitle')}</div>
            <Row className="g-2">
              {(
                [
                  {
                    flag: '🏴󠁧󠁢󠁷󠁬󠁳󠁿',
                    place: 'Pays de Galles',
                    desc: "Future Generations Commissioner (2015) — droit de regard sur toute loi affectant l'avenir.",
                  },
                  {
                    flag: '🇫🇮',
                    place: 'Finlande',
                    desc: 'Committee for the Future (1993) — comité parlementaire permanent sur les décisions à long terme.',
                  },
                  {
                    flag: '🇭🇺',
                    place: 'Hongrie',
                    desc: 'Ombudsman pour les générations futures (2007) — supprimé en 2012 sous Orbán.',
                  },
                  {
                    flag: '🌍',
                    place: 'ONU',
                    desc: 'Proposition "Déclaration des générations futures" — actuellement en discussion.',
                  },
                ] as const
              ).map(({ flag, place, desc }) => (
