...
const ResultCard: React.FC<ResultCardProps> = ({
  ...
  return (
    ...
    >
      <CardHeader
        className="block space-y-0 border-b border-border px-4 py-2 py-1"
        ...
        <div className="flex gap-1 flex-wrap mb-2">
          ...
          {result.voter_profile && (
            <Badge variant="light" style={{ fontSize: '0.7rem' }}>
              μ: {result.voter_profile.mean_ideology_x >= 0 ? '+' : ''}
              {result.voter_profile.mean_ideology_x.toFixed(2)}
            </Badge>
          )}
        </div>
        <div className="font-semibold mb-1" style={{ fontSize: '0.78rem', color: accent }}>
        ...
        <Col xs={12} md={9}>
          ...
          <Row className="g-2 mb-3 items-end">
            <Col xs={6} md={3}>
              ...
              <Range
                data-testid="vol-turnout-slider"
                min={0.4}
                max={0.9}
                step={0.05}
                value={volTurnout}
                onChange={(e) => {
                  setVolTurnout(Number(e.target.value));
                  schedule(Number(e.target.value), compTurnout, relNull, relRandom);
                }}
              />
            </Col>
            <Col xs={6} md={3}>
              ...
              <Range
                data-testid="comp-turnout-slider"
                min={0.7}
                max={0.99}
                step={0.05}
                value={compTurnout}
                onChange={(e) => {
                  setCompTurnout(Number(e.target.value));
                  schedule(volTurnout, Number(e.target.value), relNull, relRandom);
                }}
              />
            </Col>
            <Col xs={6} md={3}>
              ...
              <Range
                data-testid="null-rate-slider"
                min={0}
                max={0.15}
                step={0.01}
                value={relNull}
                onChange={(e) => {
                  setRelNull(Number(e.target.value));
                  schedule(volTurnout, compTurnout, Number(e.target.value), relRandom);
                }}
              />
            </Col>
            <Col xs={6} md={3}>
              ...
              <Range
                data-testid="random-pct-slider"
                min={0}
                max={0.5}
                step={0.05}
                value={relRandom}
                onChange={(e) => {
                  setRelRandom(Number(e.target.value));
                  schedule(volTurnout, compTurnout, relNull, Number(e.target.value));
                }}
              />
            </Col>
          </Row>
          ...
          {data && (
            <>
              ...
              <Row className="g-2 mb-3">
                <Col xs={6}>
                  <ResultCard
                    title={t('compulsory.voluntary')}
                    result={data.voluntary}
                    candidateNames={candidateNames}
                    accent="#005CAB"
                    testId="voluntary-card"
                    t={t}
                  />
                </Col>
                <Col xs={6}>
                  <ResultCard
                    title={t('compulsory.compulsoryLabel')}
                    result={data.compulsory}
                    candidateNames={candidateNames}
                    accent="#C8590A"
                    testId="compulsory-card"
                    t={t}
                  />
                </Col>
              </Row>
              ...
              {data.winner_changed && (
                <Alert variant="danger" data-testid="winner-changed-alert">
                  {t('compulsory.winnerChanged')}: {data.voluntary.winner} →{' '}
                  {data.compulsory.winner}
                </Alert>
              )}
              ...
              <Row className="g-2 mb-3" data-testid="metrics-row">
                <Col xs={12} md={6}>
                  <div className="border border-border border-success rounded p-2 h-full">
                    <div
                      className="font-semibold"
                      style={{ color: '#198754', fontSize: '0.82rem' }}
                    >
                      📈 {t('compulsory.repImprovement')}
                    </div>
                    <div style={{ fontSize: '1.2rem', fontWeight: 700, color: '#198754' }}>
                      {data.representation_improvement >= 0 ? '+' : ''}
                      {data.representation_improvement.toFixed(3)}
                    </div>
                ...
                <Col xs={12} md={6}>
                  <div className="border border-border border-danger rounded p-2 h-full">
                    <div
                      className="font-semibold"
                      style={{ color: '#dc3545', fontSize: '0.82rem' }}
                    >
          ...
          <div data-testid="examples-sidebar">
            {EXAMPLES.map((ex) => (
              ...
              >
                ...
                <div className="text-muted-foreground">{ex.system}</div>
                <Badge variant="primary" style={{ fontSize: '0.65rem' }}>
                  {ex.turnout}
                </Badge>
              </div>
            ))}
