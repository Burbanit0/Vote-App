...
type Guardrail = 'constitutional_court' | 'opposition_media' | 'international_pressure' | 'supermajority_required';

const GUARDRAILS: Guardrail[] = [
  'constitutional_court', 'opposition_media',
  'international_pressure', 'supermajority_required',
];
...
const HISTORICAL: Record<string, { label: string; color: string; data: number[] }> = {
  ...
  turkey: {
    ...
    color: '#6f42c1',
    data: [0.75, 0.71, 0.65, 0.60, 0.55, 0.50, 0.46, 0.41],
  },
  venezuela: {
    label: '🇻🇪 Venezuela (1998→2023)',
    color: '#dc3545',
    data: [0.71, 0.62, 0.52, 0.42, 0.33, 0.25, 0.20, 0.15],
  },
};
...
const QualityBar: React.FC<{ value: number }> = ({ value }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
    <div style={{
      flex: 1, height: 12, background: '#dee2e6', borderRadius: 6, overflow: 'hidden',
    }}>
      <div style={{
    ...
    <span style={{ fontSize: '0.75rem', fontWeight: 700, color: qualityColor(value), minWidth: 36 }}>
...
const DemocraticBackslidingPanel: React.FC = () => {
  ...
  // ── Chart data ─────────────────────────────────────────────────────────────
  const chartData = data ? data.elections.map((e) => {
  ...
  return (
    <div>
      {/* ── Paradox quote ── */}
      <Alert variant="danger" className="py-2 mb-3" data-testid="paradox-quote"
        style={{ fontSize: '0.78rem', borderLeft: '4px solid #dc3545' }}>
        <strong>{t('backsliding.paradoxTitle')}</strong>{' '}
        {t('backsliding.paradox')}
      </Alert>
      ...
      {/* ── Guardrails ── */}
      <div className="mb-3 border rounded p-2" style={{ background: '#f0f8ff' }}
      ...
      {!data && !loading && !error && (
        <Alert variant="secondary" data-testid="prompt-alert">{t('backsliding.prompt')}</Alert>
      )}
      ...
      {data && (
        <>
          ...
          {/* ── Timeline chart ── */}
          <div className="mb-2 fw-semibold" style={{ fontSize: '0.82rem' }}>
            {t('backsliding.timelineTitle')}
          </div>

          {/* Historical overlay toggles */}
          <div className="d-flex gap-2 flex-wrap mb-2" data-testid="historical-toggles">
            {Object.entries(HISTORICAL).map(([key, h]) => (
              <Badge key={key}
                style={{
                  cursor: 'pointer', fontSize: '0.68rem', padding: '5px 8px',
                  background: showHistorical.includes(key) ? h.color : '#dee2e6',
                  color: showHistorical.includes(key) ? '#fff' : '#495057',
                }}
                onClick={() => setShowHistorical((prev) =>
          ...
          <ResponsiveContainer width="100%" height={260} data-testid="timeline-chart">
            <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
              ...
              <XAxis dataKey="n" label={{ value: t('backsliding.electionN'), position: 'insideBottom', offset: -5, fontSize: 10 }}
                tick={{ fontSize: 10 }} />
              <YAxis domain={[0, 1]} tickFormatter={(v) => `${Math.round(v * 100)}%`}
                tick={{ fontSize: 10 }}
                label={{ value: t('backsliding.qualityAxis'), angle: -90, position: 'insideLeft', fontSize: 10 }} />
              <ReferenceLine y={0.45} stroke="#dc3545" strokeDasharray="6 3"
                label={{ value: t('backsliding.pointOfNoReturn'), fontSize: 9, fill: '#dc3545', position: 'right' }} />
              <Tooltip formatter={(v: number, name: string) => [
                `${Math.round(v * 100)}%`,
                name === 'quality' ? t('backsliding.simulationLabel') : (HISTORICAL[name]?.label ?? name),
              ]} />
              <Legend wrapperStyle={{ fontSize: '0.7rem' }} />
              <Line type="monotone" dataKey="quality"
                name={t('backsliding.simulationLabel')}
                stroke="#0d6efd" strokeWidth={2.5} dot={{ r: 3 }}
                activeDot={{ r: 5 }} />
              {showHistorical.map((hk) => (
                <Line key={hk} type="monotone" dataKey={hk}
                  name={HISTORICAL[hk].label}
                  stroke={HISTORICAL[hk].color} strokeWidth={1.5}
          ...
          <div className="mt-3" data-testid="elections-timeline">
            ...
            <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4 }}>
              {data.elections.map((e) => {
                ...
                return (
                  <div key={e.election_n}
                    style={{
                      minWidth: 80, border: `2px solid ${qualityColor(q)}`,
                      borderRadius: 6, padding: '6px 8px',
                      background: e.point_of_no_return ? '#fff3cd' : '#fff',
                      fontSize: '0.7rem',
                    }}
                    data-testid={`election-card-${e.election_n}`}>
                    <div className="fw-bold" style={{ color: qualityColor(q) }}>
                      #{e.election_n}
                    </div>
          ...
          {/* ── Advantages table ── */}
          <div className="mt-3" data-testid="advantages-section"
            style={{ fontSize: '0.75rem' }}>
            <div className="fw-semibold mb-1">{t('backsliding.advantagesTitle')}</div>
