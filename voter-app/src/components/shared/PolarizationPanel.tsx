...
const PolarHeatmap: React.FC<HeatmapProps> = ({ results }) => {
  ...
  return (
    ...
    >
      ...
      <text
        x={LABEL_W + (sorted.length - 1) * CELL_W}
        y={34}
        textAnchor="end"
        fontSize={7}
        fill="#6c757d"
  ...
  return (
    <div>
      ...
      <Row className="g-2 mb-3 items-end">
        <Col xs={12} sm={6}>
          <label className="mb-1 inline-block text-sm mb-1">{t('polarization.ideologies')}</label>
        ...
        <Col xs={12} sm={3}>
          <Button
            variant="primary"
            className="w-full"
            onClick={() => run(selectedIdeologies, numSims)}
            disabled={loading}
          >
          ...
          <Alert variant="info" className="py-2 mb-3" style={{ fontSize: '0.82rem' }}>
            ...
            <ul className="mb-0 mt-1">
              {data.key_findings.map((f, i) => (
                <li key={i} data-testid="finding-item">
                  {f}
                </li>
              ))}
            </ul>
                    ...
                    <span key={r.ideology}>
                      <span
                        style={{
                          ...
                          height: 8,
                          borderRadius: '50%',
                          background: IDEOLOGY_COLORS[r.ideology],
                          marginRight: 3,
                        }}
                      />
                      {r.ideology}: P={r.polarization_index.toFixed(2)} · CW{' '}
                      {Math.round(r.condorcet_rate * 100)}%
                    </span>
                  ))}
                </div>
              </div>
