...
interface ShyData {
  ...
  poll_results:              { poll_n: number; predicted: Record<string, number>; real: Record<string, number> }[];
...
const HISTORICAL = [
  { year: '1982', country: '🇺🇸', event: 'Bradley Effect', detail: 'Tom Bradley leading polls by 22%, lost by 1.5%' },
  { year: '1992', country: '🇬🇧', event: 'Shy Tory', detail: 'Conservateurs sous-estimés de 8 pts — victoire surprise Major' },
  { year: '2016', country: '🇬🇧', event: 'Brexit', detail: 'Leave sous-estimé de 4 pts dans les derniers sondages' },
  { year: '2002–', country: '🇫🇷', event: 'Phénomène Le Pen', detail: 'RN systématiquement sous-estimé de 2-5 pts' },
];
...
const ShyVoterPanel: React.FC = () => {
  ...
  const runSimulation = useCallback(async (factor: number, idx: number) => {
    ...
    setError(null);
    try {
      const res = await axios.post(`${API}/api/election/shy-voter`, {
  ...
  const timelineData = data
    ? [
        ...data.poll_results.map((pr) => ({
          x:    pr.poll_n,
          type: 'poll' as const,
          ...Object.fromEntries(candidateNames.map((c) => [c, Math.round((pr.predicted[c] ?? 0) * 100)])),
        })),
        {
          x:    data.poll_results.length + 1,
          type: 'real' as const,
          ...Object.fromEntries(candidateNames.map((c) => [c, Math.round((data.real_results[c] ?? 0) * 100)])),
        },
      ]
    : [];
    ...
    <div>
      ...
      <Row className="g-2 mb-3 align-items-end">
        <Col xs={12} md={4}>
          ...
          />
          <div className="d-flex justify-content-between" style={{ fontSize: '0.68rem', color: '#6c757d' }}>
      ...
      {!data && !loading && !error && (
        <Alert variant="info" role="alert">{t('shyVoter.prompt')}</Alert>
      )}
                ...
                {candidateNames.map((c, i) => (
                  <Line
                    ...
                    dot={(props: any) => {
                      ...
                      if (!isReal) return <circle key={props.key} cx={props.cx} cy={props.cy} r={2} fill={candColor(c, candidateNames)} />;
                      return <circle key={props.key} cx={props.cx} cy={props.cy} r={5} fill={candColor(c, candidateNames)} stroke="#fff" strokeWidth={2} />;
                    }}
                  />
                ))}
              </LineChart>
              ...
              {candidateNames.map((c) => {
                ...
                return (
                  <tr key={c}>
                    ...
                    <td style={{ fontSize: '0.78rem' }}>{Math.round((data.real_results[c] ?? 0) * 100)}%</td>
                    <td style={{ fontSize: '0.78rem', color: err < 0 ? '#dc3545' : err > 0 ? '#198754' : '#6c757d', fontWeight: 600 }}>
                      {err >= 0 ? '+' : ''}{Math.round(err * 100)}pp
                    </td>
                  </tr>
                );
              })}
        ...
        <div className="d-flex flex-wrap gap-2">
          {HISTORICAL.map((h) => (
            <div key={h.year}
              ...
              <div><strong>{h.country} {h.year}</strong> — {h.event}</div>
