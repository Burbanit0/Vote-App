  ...
  return (
    <div className="text-center">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxWidth: 240 }}>
        ...
        <line
          x1={hemicycleArc(cx, cy, rI - 3, Math.PI / 2)[0]}
          y1={hemicycleArc(cx, cy, rI - 3, Math.PI / 2)[1]}
          x2={hemicycleArc(cx, cy, rO + 3, Math.PI / 2)[0]}
          y2={hemicycleArc(cx, cy, rO + 3, Math.PI / 2)[1]}
          stroke="#dc3545"
          strokeWidth={1.5}
          strokeDasharray="3 2"
        />
          ...
          .map((n) => {
            ...
            return (
              <div key={n} className="flex items-center gap-1">
                ...
                {transfer > 0.005 && (
                  <span style={{ fontSize: '0.65rem', color: '#007A33' }}>
                    +{transfer.toFixed(1)}
                  </span>
                )}
              </div>
  ...
  return (
    <div>
      ...
      <Row className="g-2 mb-3 items-end">
        ...
        <Col xs={12} sm={3}>
          <label className="mb-1 inline-block text-sm mb-0">{t('stv.quotaType')}</label>
          <Select
            size="sm"
            value={quotaType}
        ...
        <>
          ...
          <Row className="g-3">
            ...
            <Col xs={12} lg={6}>
              ...
              <div className="mb-2">
                {names.map((n) => {
                  const tally = currentRound?.tallies[n] ?? (stepIdx === 0 ? initialTallies[n] : 0);
                  ...
                  return (
                    ...
                    >
                      <span
                        style={{ minWidth: 65, fontSize: '0.72rem', color: candColor(n, names) }}
                      >
                        {n}
                      </span>
                      <Progress
                ...
                {names.map((n) => {
                  ...
                  return (
                    <div key={n} className="flex gap-2 items-center mb-1">
                      ...
                      <span>→</span>
                      <span>
                        {data.stv.seats[n] ?? 0} {t('stv.seats')}
                      </span>
                      <span
                        style={{
                          color: diff > 0 ? '#007A33' : diff < 0 ? '#dc3545' : '#6c757d',
                          fontSize: '0.7rem',
                        }}
                      >
                        {diff >= 0 ? `+${diff}` : `${diff}`}pp
