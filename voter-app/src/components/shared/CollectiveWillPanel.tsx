...
const WillOMeter: React.FC<{ score: number; uniqueCount: number }> = ({ score, uniqueCount }) => {
  ...
  const arcPath = (startDeg: number, endDeg: number, color: string) => {
    ...
    return (
      <path d={`M ${x1} ${y1} A ${R} ${R} 0 0 0 ${x2} ${y2}`}
  ...
  return (
    <svg data-testid="will-o-meter" width={size} height={size * 0.72}
      ...
      {/* Labels */}
      <text x={8}    y={cy + 8} style={{ fontSize: 9, fill: '#dc3545' }}>Schumpeter</text>
      <text x={size - 10} y={cy + 8} textAnchor="end"
        style={{ fontSize: 9, fill: '#198754' }}>Rousseau</text>
      {/* Score */}
        ...
        <tbody>
          {Object.entries(byMethod).map(([method, winner]) => (
            <tr key={`m-${method}`} data-testid={`grid-method-${method}`}>
              ...
              <td className="text-muted">{t('will.typeMethod')}</td>
            </tr>
          ))}
          {Object.entries(byAgenda).map(([agenda, winner]) => (
            <tr key={`a-${agenda}`} data-testid={`grid-agenda-${agenda.slice(0,10)}`}>
              <td style={{ fontSize: '0.65rem', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {agenda}
              </td>
              <td>
                <span className="badge"
              ...
              <td className="text-muted">{t('will.typeAgenda')}</td>
            </tr>
          ))}
        </tbody>
      </table>
      {/* Color legend */}
      <div className="d-flex gap-2 flex-wrap" style={{ fontSize: '0.65rem' }}>
        {winnerList.map((w, i) => (
          <span key={w}>
            <span style={{
              display: 'inline-block', width: 10, height: 10,
              borderRadius: '50%', background: candColor(w, i), marginRight: 3,
            }} />
            {w}
          </span>
        ))}
...
const PhilosophersSection: React.FC<{ score: number; t: (k: string) => string }> = ({
  ...
  const mostRight =
    score > 0.8 ? 'rousseau' :
    score < 0.3 ? 'schumpeter' :
    score < 0.5 ? 'arrow' : 'rawls';
          ...
          <Col key={key} xs={12} md={6}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span className="text-muted ms-auto" style={{ fontSize: '0.65rem' }}>{year}</span>
              </div>
              <div className="text-muted" style={{ fontSize: '0.7rem', marginTop: 3 }}>
    ...
    <div>
      ...
      {data && (
        <>
          ...
          {/* ── Result grid ── */}
          <div className="mb-3">
            <ResultGrid byMethod={data.winner_by_method}
              byAgenda={data.winner_by_agenda} t={t} />
          </div>
          ...
          <Alert variant={score > 0.7 ? 'success' : score > 0.4 ? 'warning' : 'danger'}
            className="mb-3" data-testid="philo-conclusion">
            <strong>{t('will.conclusionTitle')}</strong> {data.philosophical_conclusion}
          </Alert>

          {/* ── The conclusion that doesn't conclude ── */}
          <div className="border rounded p-3" data-testid="open-conclusion"
