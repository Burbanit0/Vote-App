...
const DonutTooltip = ({ active, payload }: any) => {
  ...
  return (
    ...
    >
      ...
      <div className="text-muted mt-1">{methods.join(' · ')}</div>
    </div>
  );
};
...
const CentreLabel = ({
  cx, cy, singleGroup, winner, total, t,
}: {
  cx: number; cy: number;
  singleGroup: boolean;
  winner?: string;
  ...
  <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central">
    {singleGroup ? (
      <>
        <tspan fontSize={18} fill="#007A33">✓</tspan>
      </>
    ) : (
      <>
  ...
  if (single) {
    ...
    return (
      <div className="d-flex align-items-center gap-3">
        ...
        {/* Single-group info */}
        <div>
          <div className="fw-semibold" style={{ fontSize: '0.85rem', color }}>
            {singleGroup.winner}
          </div>
