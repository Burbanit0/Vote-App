...
const E2EVDemo: React.FC<Props> = ({ candidates = DEFAULT_CANDIDATES, seed = 42 }) => {
    ...
    <div data-testid="e2ev-demo">
      ...
      {step === 0 && (
        <div>
          ...
          <label className="mb-1 inline-block font-semibold" style={{ fontSize: '0.9rem' }}>
      ...
      {step === 1 && myVoter && (
        <div data-testid="step1-content">
          <Alert variant="success">
            <div className="font-bold mb-1">{t('e2ev.step1Done')}</div>
          ...
          <Card className="mb-3" style={{ background: '#f8f9fa' }}>
            <CardBody>
              ...
              <div className="text-muted-foreground mt-2" style={{ fontSize: '0.75rem' }}>
                {t('e2ev.codeExplain')}
              </div>
            </CardBody>
          </Card>
      ...
      {step === 2 && data && (
        <div data-testid="step2-content">
          ...
          <div
            ...
            style={{ background: '#f8f9fa', maxHeight: 260, overflowY: 'auto' }}
              ...
              return (
                <div
                  ...
                  className={`d-flex gap-2 align-items-center py-1 px-2 mb-1 rounded ${
                    isMyCode ? 'border border-border border-warning' : ''
                  }`}
                  style={{
                  ...
                  onClick={() => {
                    if (isMyCode) setCodeFound(true);
                  }}
                >
                  <code style={{ color: '#6f42c1', flex: 1 }}>🔒 {entry.encrypted_ballot}</code>
                  <span
                    style={{ fontFamily: 'monospace', color: isMyCode ? '#856404' : '#6c757d' }}
                  >
      ...
      {step === 3 && data && (
        <div data-testid="step3-content">
          ...
          <div className="flex flex-wrap gap-3 mb-3" data-testid="aggregate-display">
            {Object.entries(data.aggregate_result)
              .sort((a, b) => b[1] - a[1])
              .map(([cand, count]) => (
                <div key={cand} className="text-center border border-border rounded p-2">
                  <div style={{ fontWeight: 700, fontSize: '0.85rem' }}>{cand}</div>
                  <div style={{ fontSize: '1.6rem', color: '#005CAB', fontWeight: 900 }}>
                    {count}
                  </div>
                  {cand === data.winner && (
      ...
      {step === 4 && myVoter && (
        <div data-testid="step4-content">
          ...
          {!revealed && (
            <Button variant="warning" onClick={() => setRevealed(true)} data-testid="reveal-btn">
              🔓 {t('e2ev.revealMyVote')}
            </Button>
          )}
          ...
          {revealed && (
            <Alert variant="success" data-testid="revealed-result">
              <div className="font-bold">{t('e2ev.revealResult')}</div>
              <div style={{ fontSize: '0.85rem' }}>
                {t('e2ev.yourBallot')}: <code>🔒 {myVoter.encrypted_ballot}</code>
                <br />
                {t('e2ev.decrypts')}: <strong>{myHidden}</strong>
                <br />
                {t('e2ev.codeWas')}: <code>{myCode}</code>
                <br />
                {t('e2ev.consistent')}: ✓
              </div>
            </Alert>
          ...
          <Button variant="outline-danger" size="sm" onClick={reset} className="mt-2 block">
            {t('e2ev.reset')}
          </Button>
        </div>
      )}
