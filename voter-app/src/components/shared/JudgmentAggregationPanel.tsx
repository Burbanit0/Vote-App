...
const LogicTreeSVG: React.FC<TreeProps> = ({ data, incoherences }) => {
  ...
  return (
    <svg data-testid="logic-tree-svg" width={SVG_W} height={SVG_H} style={{ display: 'block' }}>
      {/* Premise nodes */}
      {premises.map((p, i) => {
        ...
        return (
          <g key={p.id}>
            <rect
              ...
              fill={col}
              fillOpacity={0.15}
              stroke={isInco ? '#dc3545' : col}
              strokeWidth={isInco ? 2.5 : 1.5}
            />
            <text
              x={px + 65}
              y={py + 4}
            ...
            {conclusions.map((c, ci) => {
              ...
              return (
                <line
                  ...
                  y2={cy}
                  stroke={edgeCol}
                  strokeWidth={isInco && incoSet.has(c.id) ? 2 : 1}
          ...
          <g key={c.id}>
            ...
            {isInco && (
              <text
                ...
                y={cy + 19}
                textAnchor="middle"
                style={{ fontSize: 8, fill: '#dc3545', fontWeight: 700 }}
              >
                ⚡ INCOHÉRENT
              </text>
            )}
  ...
  return (
    <div>
      ...
      <Row className="g-2 mb-3 items-end">
        <Col xs={12} md={3}>
          <label className="mb-1 inline-block text-sm mb-0">{t('judg.scenario')}</label>
          <Select
            size="sm"
            value={scenario}
        ...
        <Col xs={6} md={2}>
          <label className="mb-1 inline-block text-sm mb-0">{t('judg.numVoters')}</label>
          <Control
            type="number"
            size="sm"
            min={1}
        ...
        <Col xs={6} md={2}>
          <label className="mb-1 inline-block text-sm mb-0">{t('judg.seed')}</label>
          <Control
            type="number"
            size="sm"
            value={seed}
          ...
          <Row className="g-3">
            <Col xs={12} md={7}>
              ...
              <Table
                ...
                data-testid="propositions-table"
              >
                <thead>
                  <tr>
                ...
                <tbody>
                  {data.propositions.map((p) => {
                    ...
                    return (
                      <tr key={p.id} style={{ background: isInco ? '#fff5f5' : undefined }}>
                        <td style={{ fontSize: '0.78rem' }}>
                          <Badge
                            variant={p.type === 'premise' ? 'secondary' : 'primary'}
                        ...
                        <td style={{ fontSize: '0.78rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <div
                              style={{
                                width: `${Math.round(p.yes_pct * 60)}px`,
                                height: 8,
                        ...
                        <td style={{ fontSize: '0.78rem' }}>
                          <span
                            style={{
                              color: p.collective_vote ? '#198754' : '#dc3545',
                              fontWeight: 700,
                            }}
                          >
                ...
                <Card className="border-warning mb-2" data-testid="resolution-card">
                  ...
                  <CardBody className="py-2">
                    <div className="flex gap-4" style={{ fontSize: '0.78rem' }}>
                      <div>
                        <div className="text-muted-foreground">{t('judg.premiseBased')}</div>
                      ...
                      <div>
                        <div className="text-muted-foreground">{t('judg.conclusionBased')}</div>
                        <Badge variant="secondary">{t('judg.overridePremise')}</Badge>
                      </div>
                    </div>
                  </CardBody>
                </Card>
