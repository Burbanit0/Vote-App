...
interface DemoData {
  ...
  representation_gap: { ideology_drift: number; overrepresented_groups: string[]; underrepresented_groups: string[] };
...
interface DemoProfile { label: string; age_distribution: number[]; turnout_by_age: number[]; ideology_by_age: number[]; education_distribution: number[]; turnout_by_education: number[]; ideology_by_education: number[]; }
...
const PROFILES: Record<string, DemoProfile> = {
  france_2022: {
    ...
    ideology_by_education:  [0.10, -0.05],
  },
  usa_2020: {
    label: 'USA 2020',
    age_distribution: [0.22, 0.48, 0.30],
    turnout_by_age:   [0.54, 0.65, 0.76],
    ideology_by_age:  [-0.20, 0.0, 0.25],
    education_distribution: [0.40, 0.60],
    turnout_by_education:   [1.0, 1.0],
    ideology_by_education:  [0.15, -0.10],
  },
  sweden_2022: {
    label: 'Suède 2022',
    age_distribution: [0.20, 0.50, 0.30],
    turnout_by_age:   [0.75, 0.85, 0.88],
    ideology_by_age:  [-0.10, 0.0, 0.10],
  ...
  uk_2019: {
    ...
    ideology_by_age:  [-0.25, 0.0, 0.30],
    education_distribution: [0.42, 0.58],
    turnout_by_education:   [1.0, 1.0],
    ideology_by_education:  [0.20, -0.15],
  },
};

// ── Ideology drift SVG ────────────────────────────────────────────────────────

const DRIFT_W = 400, DRIFT_H = 60, DRIFT_PAD = 30;

interface DriftProps { fullMean: number; biasedMean: number; threshold: number; }
...
const IdeologyDriftSVG: React.FC<DriftProps> = ({ fullMean, biasedMean }) => {
  ...
  return (
    <svg data-testid="ideology-drift-svg" width="100%" viewBox={`0 0 ${DRIFT_W} ${DRIFT_H}`}>
      {/* Axis */}
      <line x1={DRIFT_PAD} y1={DRIFT_H / 2} x2={DRIFT_W - DRIFT_PAD} y2={DRIFT_H / 2} stroke="#dee2e6" strokeWidth={2} />
      <text x={DRIFT_PAD} y={DRIFT_H - 4} textAnchor="middle" style={{ fontSize: 9, fill: '#6c757d' }}>G</text>
      <text x={DRIFT_W - DRIFT_PAD} y={DRIFT_H - 4} textAnchor="middle" style={{ fontSize: 9, fill: '#6c757d' }}>D</text>
      {/* Full population mean */}
      <line x1={pct(fullMean)} y1={8} x2={pct(fullMean)} y2={DRIFT_H - 12} stroke="#adb5bd" strokeWidth={2} strokeDasharray="4 2" />
      <text x={pct(fullMean)} y={7} textAnchor="middle" style={{ fontSize: 8, fill: '#6c757d' }}>Population</text>
      {/* Biased mean */}
      <line x1={pct(biasedMean)} y1={8} x2={pct(biasedMean)} y2={DRIFT_H - 12} stroke="#dc3545" strokeWidth={2} />
      <text x={pct(biasedMean)} y={7} textAnchor="middle" style={{ fontSize: 8, fill: '#dc3545' }}>Votants</text>
    </svg>
  );
};
...
const DemographicTurnoutPanel: React.FC = () => {
  ...
  // ── Chart data ──────────────────────────────────────────────────────────
  const pyramidData = data?.demographic_breakdown.map((d) => ({
  ...
  return (
    <div>
        ...
        <Col xs={12} md={6}>
          <div className="d-flex flex-wrap gap-1" style={{ fontSize: '0.72rem' }}>
            {['jeunes', 'adultes', 'seniors'].map((ag, i) => (
              <div key={ag} className="border rounded px-2 py-1">
      ...
      {!data && !loading && !error && (
        <Alert variant="info" role="alert">{t('demo.prompt')}</Alert>
      )}
      ...
      {data && (
        <>
          ...
          <div className="mb-3">
            <div className="fw-semibold mb-1" style={{ fontSize: '0.85rem' }}>{t('demo.ideologyTitle')}</div>
            <IdeologyDriftSVG
          ...
          <div className="mb-4" data-testid="pyramid-chart">
            <div className="fw-semibold mb-1" style={{ fontSize: '0.85rem' }}>{t('demo.pyramidTitle')}</div>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={pyramidData} layout="vertical"
                ...
                <Bar dataKey="population" name={t('demo.population')} fill="#adb5bd" opacity={0.6} />
          ...
          <Row className="g-3 mb-4">
            <Col xs={6}>
              <div className="fw-semibold mb-1" style={{ fontSize: '0.82rem' }}>{t('demo.biasedResult')}</div>
            ...
            <Col xs={6}>
              <div className="fw-semibold mb-1" style={{ fontSize: '0.82rem' }}>{t('demo.correctedResult')}</div>
              <div data-testid="corrected-bar-chart">
                <ResponsiveContainer width="100%" height={140}>
                  <BarChart data={correctedBarData} margin={{ top: 4, right: 8, bottom: 4, left: 0 }}>
                    <XAxis dataKey="name" tick={{ fontSize: 10 }} />
                    <YAxis unit="%" tick={{ fontSize: 10 }} domain={[0, 60]} />
                    <Tooltip formatter={(v: number) => `${v}%`} />
                    <Bar dataKey="share">
          ...
          <Table size="sm" hover className="mb-3" data-testid="breakdown-table">
            ...
            <tbody>
              {data.demographic_breakdown.map((d) => {
                ...
                return (
                  <tr key={d.group} style={{ background: over ? '#f0fff4' : under ? '#fff5f5' : undefined }}>
                    <td style={{ fontSize: '0.78rem' }}>{d.group}</td>
                    <td style={{ fontSize: '0.78rem' }}>{Math.round(d.population_pct * 100)}%</td>
                    <td style={{ fontSize: '0.78rem', fontWeight: 600, color: over ? '#198754' : under ? '#dc3545' : undefined }}>
                      {Math.round(d.voter_pct * 100)}%
                      {over  && ' ▲'}
                      {under && ' ▼'}
                    </td>
                    <td style={{ fontSize: '0.78rem' }}>{d.ideology_mean >= 0 ? '+' : ''}{d.ideology_mean.toFixed(2)}</td>
                  </tr>
                );
              })}
            </tbody>
