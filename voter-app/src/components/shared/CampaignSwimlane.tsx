...
const CampaignSwimlane: React.FC<Props> = ({ snapshots, methodStability, candidates }) => {
  ...
  const sortedMethods = useMemo(
    () =>
      Object.keys(methodStability).sort(
        (a, b) =>
          (methodStability[b]?.stability_score ?? 0) - (methodStability[a]?.stability_score ?? 0)
      ),
    [methodStability]
  );
  ...
  const methodsForSelected = useMemo(() => {
    ...
    return sortedMethods.filter((m) => methodStability[m]?.final_winner === selectedCand);
  ...
  return (
    <div>
      {/* Click hint */}
      <p className="text-muted-foreground mb-1" style={{ fontSize: '0.72rem' }}>
            ...
            return (
              <g key={s.day}>
                <line
                  ...
                  strokeWidth={1}
                />
                <text x={x} y={MARGIN_TOP - 8} textAnchor="middle" fontSize={9} fill={tickColor}>
                  J{s.day}
                </text>
              </g>
            );
            ...
            return (
              <g key={method}>
                ...
                {snapshots.map((snap, si) => {
                  ...
                  const x2 =
                    si < snapshots.length - 1 ? dayToX(snapshots[si + 1].day) : LABEL_W + PLOT_W;
                  ...
                  return (
                    <rect
                      ...
                      opacity={opBase}
                      style={{
                        transition: mounted ? 'opacity 0.25s' : 'opacity 0.5s',
                        cursor: 'pointer',
                      }}
      ...
      {hovered && (
        <div
          ...
          style={{ fontSize: '0.74rem', minHeight: '1.2em' }}
        >
      ...
      <div className="flex gap-3 mt-2 flex-wrap" style={{ fontSize: '0.72rem' }}>
        {candidates.map((cand, idx) => (
          <span
            ...
            style={{
              cursor: 'pointer',
              opacity: selectedCand === null || selectedCand === cand ? 1 : 0.4,
            }}
      ...
      {selectedCand && (
        <div className="flex items-center gap-2 mt-2 flex-wrap" style={{ fontSize: '0.75rem' }}>
          <Badge style={{ background: candColorMap[selectedCand], fontSize: '0.72rem' }}>
            {selectedCand}
          </Badge>
          {methodsForSelected.length > 0 ? (
            <>
              <span className="text-muted-foreground">{t('campaign.swimlaneElectedBy')}:</span>
              {methodsForSelected.map((m) => (
                <Badge key={m} variant="secondary" style={{ fontSize: '0.68rem' }}>
                  {m}
                </Badge>
              ))}
            </>
          ) : (
            <span className="text-muted-foreground">{t('campaign.swimlaneNoFinal')}</span>
          )}
          <span
