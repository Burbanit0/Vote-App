...
const PolisScatter: React.FC<ScatterProps> = ({ positions }) => {
  ...
  const px = (x: number) => SCATTER_PAD + ((x - minX) / spanX) * (SCATTER_W - 2 * SCATTER_PAD);
  const py = (y: number) =>
  ...
  return (
    <div>
      ...
      {data.consensus_statements.length > 0 && (
        <div
          className="border border-border border-success rounded p-3 mb-3"
          data-testid="consensus-section"
        >
          <div className="font-semibold mb-2" style={{ color: '#198754', fontSize: '0.85rem' }}>
            ✅ {t('tech.consensusTitle')}
          </div>
          <div className="text-muted-foreground mb-2" style={{ fontSize: '0.75rem' }}>
      ...
      {data.polarizing_statements.length > 0 && (
        <div
          className="border border-border border-danger rounded p-3 mb-3"
          data-testid="polarizing-section"
        >
          <div className="font-semibold mb-2" style={{ color: '#dc3545', fontSize: '0.85rem' }}>
              ...
              <div className="flex gap-1">
                {data.clusters.map((c) => {
                  ...
                  return (
                    <div
                      key={c.id}
                      title={`${t('tech.polisCluster')} ${c.id + 1}: ${Math.round(rate * 100)}%`}
                      style={{
