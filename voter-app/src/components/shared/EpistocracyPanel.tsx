...
const CompetenceHistogram: React.FC<HistogramProps> = ({
  mean, std, expertPct, threshold, dist,
}) => {
  const W = 320, H = 130, padL = 30, padB = 20, padR = 10, padT = 10;
  ...
  const barData = Array.from({ length: bins }, (_, i) => {
    ...
    if (dist === 'bimodal') {
      ...
      const g2 = Math.exp(-((x - (mean + 0.20)) ** 2) / (2 * (std * 0.7) ** 2)) * 0.4;
      density = g1 + g2;
  ...
  return (
    <svg data-testid="competence-histogram" width={W} height={H}
      ...
      {barData.map((d, i) => {
        ...
        return (
          <rect key={i} x={bx} y={by} width={barW - 1} height={bh}
            fill={color} opacity={0.7} />
        );
      })}
      {/* P=0.5 reference line */}
      <line x1={padL + chartW * 0.5} y1={padT}
            x2={padL + chartW * 0.5} y2={padT + chartH}
            stroke="#dc3545" strokeWidth={2} strokeDasharray="4 3" />
      <text x={padL + chartW * 0.5 + 2} y={padT + 10}
        style={{ fontSize: 9, fill: '#dc3545' }}>P=0.5</text>
      {/* Mean line */}
      <line x1={padL + chartW * mean} y1={padT}
            x2={padL + chartW * mean} y2={padT + chartH}
            stroke="#333" strokeWidth={1.5} />
      {/* Threshold line */}
      <line x1={padL + chartW * threshold} y1={padT}
            x2={padL + chartW * threshold} y2={padT + chartH}
            stroke="#0d6efd" strokeWidth={1} strokeDasharray="3 2" />
      {/* Axis */}
      <line x1={padL} y1={padT + chartH} x2={padL + chartW} y2={padT + chartH}
        stroke="#adb5bd" />
      <text x={padL} y={H - 4} style={{ fontSize: 9, fill: '#6c757d' }}>0</text>
      <text x={padL + chartW - 6} y={H - 4} style={{ fontSize: 9, fill: '#6c757d' }}>1</text>
...
const EpistocracyPanel: React.FC<EpistocracyLabProps> = ({
  ...
  const [expertPct,    setExpertPct]    = useState(0.10);
  ...
  return (
    <div>
      {/* ── Caplan quote ── */}
      <Alert variant="warning" className="py-2 mb-3" data-testid="caplan-quote"
        style={{ fontSize: '0.78rem', borderLeft: '4px solid #ffc107' }}>
        <strong>{t('episto.caplanQuoteTitle')}</strong>{' '}
      ...
      <div className="mb-3" data-testid="histogram-section">
        ...
        <CompetenceHistogram
          mean={compMean} std={compStd}
          expertPct={expertPct} threshold={threshold} dist={compDist}
        />
        <div className="d-flex gap-3 mt-1" style={{ fontSize: '0.65rem' }}>
          <span><span style={{ color: '#dc3545' }}>■</span> P&lt;0.5 (nuisible)</span>
          <span><span style={{ color: '#198754' }}>■</span> 0.5≤P&lt;seuil</span>
          <span><span style={{ color: '#0d6efd' }}>■</span> P≥seuil (épistocracie)</span>
        </div>
      </div>

      {!data && !loading && !error && (
        <Alert variant="secondary" data-testid="prompt-alert">{t('episto.prompt')}</Alert>
      )}
            ...
            <div data-testid="quality-view">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={qualityChartData}
                  ...
                  <ReferenceLine y={33} stroke="#adb5bd" strokeDasharray="4 3"
              ...
              {/* Condorcet curve */}
              <div className="fw-semibold mt-3 mb-1" style={{ fontSize: '0.82rem' }}>
                {t('episto.condorcetTitle')}
              </div>
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={condorcetCurve}
                  margin={{ top: 5, right: 20, bottom: 20, left: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="p" unit="%" tick={{ fontSize: 10 }}
                    label={{ value: 'P moyen', position: 'insideBottom', offset: -10, fontSize: 10 }} />
                  <YAxis tick={{ fontSize: 10 }} unit="%" domain={[0, 100]} />
                  <ReferenceLine y={33} stroke="#dc3545" strokeDasharray="4 3"
                    label={{ value: t('episto.randomChance'), fontSize: 9, fill: '#dc3545', position: 'right' }} />
                  <ReferenceLine x={Math.round(compMean * 100)}
                    stroke="#333" strokeWidth={2}
                    label={{ value: t('episto.currentP'), fontSize: 9, fill: '#333', position: 'top' }} />
                  <Tooltip formatter={(v: number) => `${v}%`} />
                  <Line type="monotone" dataKey="acc" name={t('episto.collectiveAcc')}
                  ...
                  {SCHEMES.map(s => {
                    ...
                    return (
                      <tr key={s}
                        ...
                        <td style={{ fontWeight: 700, color: r.correct_choice_pct < 0.33 ? '#dc3545' : undefined }}>
                          {Math.round(r.correct_choice_pct * 100)}%
                        </td>
                        <td>{Math.round(r.bayesian_regret * 100)}%</td>
                        <td>
                          {s === 'epistocratic'
                            ? <span style={{ color: r.participates_pct < 0.2 ? '#dc3545' : '#198754' }}>
          ...
          <Row className="g-2 mt-2" data-testid="arguments-section">
            <Col xs={12} md={6}>
              <div className="border rounded p-2"
                ...
                <div className="fw-semibold mb-1">⚠ {t('episto.forTitle')}</div>
            ...
            <Col xs={12} md={6}>
              <div className="border rounded p-2"
                ...
                <div className="fw-semibold mb-1">✓ {t('episto.againstTitle')}</div>
                <ul className="mb-0" style={{ paddingLeft: 14 }}>
