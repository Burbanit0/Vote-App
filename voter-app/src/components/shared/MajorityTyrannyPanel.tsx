...
const MajorityTyrannyPanel: React.FC = () => {
  ...
  const [majorityPct, setMajorityPct] = useState(0.6);
  ...
  return (
    <div>
      ...
      <Alert
        variant="danger"
        className="py-2 mb-3"
        data-testid="tocqueville-quote"
        style={{ fontSize: '0.78rem', borderLeft: '4px solid #dc3545' }}
      >
        <strong>{t('tyranny.hitlerQuoteTitle')}</strong> {t('tyranny.hitlerQuote')}
      </Alert>

      {/* ── Controls ── */}
      <Row className="g-2 mb-3 items-end">
        <Col xs={6} md={2}>
          <label className="mb-1 inline-block text-sm mb-0">{t('tyranny.voters')}</label>
          <Control
            type="number"
            size="sm"
            min={20}
        ...
        <Col xs={6} md={3}>
          ...
          <Range
            min={51}
            max={80}
            value={Math.round(majorityPct * 100)}
        ...
        <Col xs={6} md={2}>
          <label className="mb-1 inline-block text-sm mb-0">{t('tyranny.decisions')}</label>
          <Control
            type="number"
            size="sm"
            min={10}
        ...
        <Col xs={6} md={1}>
          <label className="mb-1 inline-block text-sm mb-0">{t('tyranny.seed')}</label>
          <Control
            type="number"
            size="sm"
            value={seed}
        ...
        <Col xs="auto">
          <Button variant="danger" onClick={run} disabled={loading} data-testid="run-btn">
      ...
      {/* ── Rule toggles ── */}
      <div className="flex flex-wrap gap-2 mb-3" data-testid="rule-toggles">
        {ALL_RULES.map((r) => (
          <Badge
            key={r}
      ...
      {data && (
        <>
          {/* ── Summary badges ── */}
          <div className="flex flex-wrap gap-2 mb-3" data-testid="summary-badges">
          ...
          {activeView === 'radar' && (
            <div data-testid="radar-view">
              ...
              <ResponsiveContainer width="100%" height={320}>
                <RadarChart
                  data={buildRadarData(data.results)}
          ...
          {activeView === 'curve' && (
            <div data-testid="curve-view">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <LineChart
                  data={data.tyranny_curve.map((pt) => ({
          ...
          {activeView === 'table' && (
            <div data-testid="table-view" style={{ overflowX: 'auto' }}>
              <table className="table table-sm table-bordered" style={{ fontSize: '0.76rem' }}>
                        ...
                        >
                          ...
                          <td>
                            <span
                              style={{
                                color:
                                  res.tyranny_index > 0.8
                                    ? '#dc3545'
                                    : res.tyranny_index > 0.4
                                      ? '#fd7e14'
                                      : '#198754',
                                fontWeight: 700,
                              }}
          ...
          <div
            ...
            data-testid="safeguards-section"
            style={{ background: '#f8f9fa', fontSize: '0.78rem' }}
          >
            <div className="font-semibold mb-2">{t('tyranny.safeguardsTitle')}</div>
            <Row className="g-2">
