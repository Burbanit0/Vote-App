...
const BALLOT_TYPES: {
  method: string; labelKey: string; visual: string; stars: number;
}[] = [
  ...
  { method: 'star_voting',       labelKey: 'ballot.typeSTAR',        visual: 'A ★★★☆☆  B ★★☆☆☆  C ★★★★☆',      stars: 3 },
  { method: 'majority_judgment', labelKey: 'ballot.typeMJ',          visual: 'A [TB]  B [AB]  C [P]',            stars: 2 },
...
const LINE_COLORS = [
  ...
  '#e67e22', '#17a2b8', '#e83e8c', '#6c757d',
];
function lineColor(i: number): string { return LINE_COLORS[i % LINE_COLORS.length]; }
...
function starsElement(n: number): string { return '★'.repeat(n) + '☆'.repeat(Math.max(0, 4 - n)); }
...
const BallotComplexityPanel: React.FC = () => {
  ...
  const schedule = useCallback((edu: number, ftv: number) => {
  ...
  // ── Curve data ──────────────────────────────────────────────────────────
  const curveData = data?.candidate_count_curve.map((pt) => ({
    n: pt.n_candidates,
    ...Object.fromEntries(
    ...
    <div>
      ...
      <Row className="g-2 mb-3 align-items-end">
        ...
        {data && data.results.length > 0 && (() => {
          ...
          data.results.forEach((r) => { winnersByMethod[r.method] = r.winner; });
      ...
      {!data && !loading && !error && (
        <Alert variant="info" role="alert">{t('ballot.prompt')}</Alert>
      )}
      ...
      {data && (
        <>
          ...
          <div className="mb-4" data-testid="candidate-curve-chart">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={curveData} margin={{ top: 4, right: 16, bottom: 4, left: 0 }}>
                ...
                <XAxis dataKey="n" label={{ value: t('ballot.nCandidates'), position: 'insideBottom', offset: -2, fontSize: 10 }} />
                <YAxis unit="%" tick={{ fontSize: 10 }} />
                <Tooltip formatter={(v: number) => `${v}%`} />
                <Legend wrapperStyle={{ fontSize: 10 }} />
                {methodKeys.map((m, i) => (
                  <Line key={m} type="monotone" dataKey={m}
                    stroke={lineColor(i)} dot={false} strokeWidth={1.5} />
                ))}
              </LineChart>
            </ResponsiveContainer>
          </div>
            ...
            <tbody>
              {data.results.map((r) => (
                <tr key={r.method} data-testid={`result-row-${r.method}`}>
                  <td style={{ fontSize: '0.78rem' }}><code>{r.method}</code></td>
          ...
          <div className="d-flex flex-wrap gap-2 mb-3" data-testid="ballot-design-section">
            {BALLOT_TYPES.filter((bt) => methodKeys.includes(bt.method)).map((bt) => {
              ...
              return (
                <div key={bt.method}
                  ...
                  <div className="fw-semibold">{bt.method}</div>
