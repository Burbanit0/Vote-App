...
const HotellingPanel: React.FC = () => {
  ...
  const bgColor = data
    ? data.equilibrium_type === 'center_convergence'
      ? '#f0fff4'
      : data.equilibrium_type === 'dispersed'
        ? '#fff8e1'
        : '#fff3f3'
    : 'transparent';
          ...
          <Col xs={12} lg={7}>
            ...
            >
              ...
              <rect
                ...
                strokeWidth={0.8}
              />
              <text x={PAD} y={SVG_H - 6} fontSize={9} fill="#adb5bd">
                ← Gauche
              </text>
              <text x={SVG_W - PAD} y={SVG_H - 6} textAnchor="end" fontSize={9} fill="#adb5bd">
                Droite →
              </text>

              {/* Voter dots */}
              {data.voters.map((v, i) => (
                <circle key={i} cx={dx(v.x)} cy={dy(v.y)} r={2} fill="#adb5bd" fillOpacity={0.3} />
              ))}
                  ...
                  return (
                    <div key={name} className="flex items-center gap-2 mb-1">
                      <span
                        style={{
                          minWidth: 65,
                          color: candColor(name, candidateNames),
                          fontSize: '0.8rem',
                        }}
                      >
                        {name}
                ...
                {Object.entries(compareData)
                  ...
                  .map(([m, d]) => (
                    <tr key={m}>
                      ...
                      <td>
                        <Badge
                          variant={d.converged ? 'success' : 'danger'}
                          style={{ fontSize: '0.65rem' }}
                        >
