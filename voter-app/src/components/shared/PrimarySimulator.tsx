...
const IdeologicalAxis: React.FC<IdeologicalAxisProps> = ({ primaries, partyColors }) => {
  ...
  return (
    ...
    >
      ...
      {/* Labels */}
      <text x={PAD} y={axisY + 16} textAnchor="middle" fontSize={10} fill="#6c757d">
        −1
      </text>
      <text x={W / 2} y={axisY + 16} textAnchor="middle" fontSize={10} fill="#6c757d">
        0
      </text>
      <text x={W - PAD} y={axisY + 16} textAnchor="middle" fontSize={10} fill="#6c757d">
        +1
      </text>

      {/* Left / Right labels */}
      <text x={PAD} y={axisY - 18} textAnchor="start" fontSize={9} fill="#6c757d">
        ← Gauche / Left
      </text>
      <text x={W - PAD} y={axisY - 18} textAnchor="end" fontSize={9} fill="#6c757d">
        Droite / Right →
      </text>
        ...
        return (
          <g key={p.party}>
            ...
            <circle cx={cx} cy={axisY} r={6} fill="#fff" stroke="#adb5bd" strokeWidth={1.5}>
              <title>
                {p.party} — centre: {p.party_center.toFixed(2)}
              </title>
            </circle>
            ...
            <circle cx={wx} cy={axisY} r={7} fill={color} stroke="#fff" strokeWidth={1.5}>
              <title>
                {p.winner} — pos: {p.winner_pos.toFixed(2)}
              </title>
            </circle>
    ...
    <div>
      {parties.map((party, pIdx) => (
        <Card
          key={pIdx}
          className="mb-2"
          style={{ fontSize: '0.78rem', borderLeft: `4px solid ${partyColor(pIdx)}` }}
        >
          <CardBody className="p-2">
            ...
            <div className="ps-2">
              {party.primary_candidates.map((c, cIdx) => (
                <div key={cIdx} className="flex items-center gap-1 mb-1">
                  <span style={{ minWidth: 80, fontSize: '0.72rem' }}>{c.name.split(' ')[0]}</span>
                  <Range
                    min={-1}
                    max={1}
                    step={0.05}
                    value={c.ideology_position}
                    onChange={(e) =>
                      updateCandidate(pIdx, cIdx, { ideology_position: Number(e.target.value) })
                    }
                    style={{ flex: 1 }}
                  />
                  <span style={{ minWidth: 32, fontSize: '0.7rem', textAlign: 'right' }}>
                    {c.ideology_position >= 0 ? '+' : ''}
                    {c.ideology_position.toFixed(2)}
                  </span>
                </div>
              ))}
            </div>
...
const PrimarySimulator: React.FC = () => {
  ...
  const maxDistortion = data ? Math.max(...data.primaries.map((p) => p.distortion), 0.001) : 1;
    ...
    <div>
      <Row className="g-3">
        ...
        <Col xs={12} md={4}>
          ...
          <Button variant="primary" className="w-full mt-2" onClick={run} disabled={loading}>
            {loading ? (
              <>
                <Spinner size="sm" className="me-2" />
                {t('primary.simulating')}
              </>
            ) : (
              `🗳 ${t('primary.run')}`
            )}
        ...
        {/* ── Right panel: results ── */}
        <Col xs={12} md={8}>
          {!data && !loading && <Alert variant="info">{t('primary.prompt')}</Alert>}
          ...
          {data && (
            <>
              ...
              <Row className="g-2 mb-3" data-testid="primaries-section">
                {data.primaries.map((p, i) => (
                  <Col key={p.party} xs={12} sm={6} lg={4}>
                    <Card
                      className="h-full"
                      style={{ fontSize: '0.78rem', borderTop: `3px solid ${partyColors[i]}` }}
                    >
                      <CardBody className="p-2">
                        ...
                        <div className="text-muted-foreground mb-1">
                          ✓ {p.winner}
                          {p.runner_up && (
                            <span className="ms-2 text-muted-foreground">({p.runner_up})</span>
                          )}
                        </div>
              ...
              <div className="border border-border rounded p-2 mb-3">
                ...
                >
                  <span>
                    ○ {t('primary.partyCenter')} &nbsp; ● {t('primary.selectedCandidate')}
                  </span>
                </div>
              </div>
              ...
              <div className="border border-border rounded p-2" data-testid="general-section">
