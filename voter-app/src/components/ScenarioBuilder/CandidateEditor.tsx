...
const CandidateEditor: React.FC<Props> = ({ candidates, onChange }) => {
  ...
  const Slider = ({
    ...
    <div className="mb-2">
      <div className="flex justify-between mb-1">
        ...
        <small className="font-semibold">{Math.round(value * 100)}%</small>
      </div>
      <Range
        min={0}
        max={1}
  ...
  return (
    <div>
      <p className="text-muted-foreground text-sm mb-3">{t('scenario.candidateEditorIntro')}</p>
            ...
            {c.isBlank ? (
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Badge variant="warning" style={{ fontSize: '0.85rem' }}>
                    {t('scenario.blankVoteBadge')}
                  </Badge>
                  <small className="text-muted-foreground">{t('scenario.blankDescription')}</small>
                </div>
                <Button variant="outline-danger" size="sm" onClick={() => remove(c.id)}>
                  ✕
                </Button>
              </div>
            ) : (
              <>
                <Row className="g-2 items-center mb-3">
                  <Col md={5}>
                    <Control
                      size="sm"
                  ...
                  <Col md={6}>
                    <div className="flex items-center gap-2">
                      <small
                        ...
                        style={{ color: '#3c3cdc', minWidth: 50 }}
                      >
                        {t('scenario.left')}
                      </small>
                      <Range
                      ...
                      <small
                        className="whitespace-nowrap"
                        style={{ color: '#dc3c3c', minWidth: 50, textAlign: 'right' }}
                      >
                        {t('scenario.right')}
                      </small>
                    </div>
                    <div className="text-center mt-1">
                      <Badge
                        style={{ backgroundColor: ideologyColor(c.ideology), fontSize: '0.72rem' }}
                      >
                ...
                <Row className="g-3">
                  <Col md={4}>
                    <Slider
                      label={t('scenario.economySlider')}
                      value={c.economy}
                      onChange={(v) => update(c.id, { economy: v })}
                    />
                  </Col>
                  <Col md={4}>
                    <Slider
                      label={t('scenario.environmentSlider')}
                      value={c.environment}
                      onChange={(v) => update(c.id, { environment: v })}
                    />
                  </Col>
                  <Col md={4}>
                    <Slider
                      label={t('scenario.socialSlider')}
                      value={c.social}
                      onChange={(v) => update(c.id, { social: v })}
                    />
                  </Col>
                </Row>
              </>
      ...
      <div className="flex gap-2">
        ...
        {!hasBlank && (
          <Button
            ...
            size="sm"
            onClick={() => onChange([...candidates, newBlankCandidate()])}
          >
            {t('scenario.addBlank')}
          </Button>
        )}
