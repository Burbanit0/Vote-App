  ...
  return (
    ...
    >
      ...
      {!data.condorcet_winner_exists && (
        <circle cx={tcx} cy={tcy} r={tcRadius}
          fill="#dc3545" fillOpacity={0.08} stroke="#dc3545" strokeWidth={1}
          strokeDasharray="6 3" />
      )}

      {/* Voter ideal points */}
      {data.voter_ideal_points.map((pt, i) => (
        <circle key={i} cx={coord(pt)[0]} cy={coord(pt)[1]} r={6}
          fill="#005CAB" opacity={0.7} />
      ))}
      ...
      {visAlt.length > 1 && (
        <>
          ...
          />
          {visAlt.map((p, i) => (
            <circle key={`alt-${i}`} cx={coord(p)[0]} cy={coord(p)[1]} r={i === 0 ? 8 : 5}
              fill={i === visAlt.length - 1 ? '#fd7e14' : '#ffc107'} opacity={0.8} />
          ))}
          <text x={coord(visAlt[visAlt.length - 1])[0] + 6}
            y={coord(visAlt[visAlt.length - 1])[1]}
            style={{ fontSize: 10, fill: '#fd7e14', fontWeight: 700 }}>Cible B</text>
        </>
      )}
      ...
      {visSteps.length > 1 && (
        <polyline
          ...
          fill="none" stroke="#198754" strokeWidth={2} opacity={0.85}
        />
      )}
      {visSteps.map((p, i) => {
        const [cx, cy] = coord(p);
        ...
        return (
          <g key={`s-${i}`}>
            <circle cx={cx} cy={cy}
              ...
              stroke={isCurrent ? '#198754' : 'none'} strokeWidth={2}
            />
            {(isStart || isEnd) && (
              <text x={cx + 8} y={cy + 4} style={{ fontSize: 10, fill: isStart ? '#005CAB' : '#198754', fontWeight: 700 }}>
      ...
      {data.condorcet_winner_exists && (
        <text x={SVG_SIZE / 2} y={PAD - 6} textAnchor="middle"
...
const PlottChaosPanel: React.FC = () => {
  ...
  return (
    <div>
      ...
      {!data && !loading && !error && (
        <Alert variant="info" role="alert">{t('plott.prompt')}</Alert>
      )}
        ...
        <Row className="g-3">
          <Col xs={12} md={7}>
            ...
            <div className="d-flex gap-2 mt-2 flex-wrap align-items-center">
              <Button size="sm" variant="outline-secondary"
                onClick={() => { setAnimStep(0); setPlaying(false); }}>⏮</Button>
              ...
              <Form.Range
                ...
                onChange={(e) => { setAnimStep(Number(e.target.value)); setPlaying(false); }}
                style={{ maxWidth: 120 }}
              />
            </div>
          </Col>
          ...
          <Col xs={12} md={5}>
            ...
            <div className="border rounded p-3 mb-3" style={{ background: '#f8f9fa' }}>
              <div style={{ fontSize: '0.8rem' }} className="mb-2">{t('plott.chaosExplain')}</div>
              <div className="d-flex gap-2">
                <Button size="sm" variant="success"
                  data-testid="show-path-a-btn"
                  onClick={() => { setShowAlt(false); setAnimStep(-1); }}>
                  🟢 {t('plott.showPathA')}
                </Button>
