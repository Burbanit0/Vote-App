  ...
  const politicalLeanData = (): ChartData[] => {
    ...
    return voters.reduce((bins, voter) => {
      ...
      return bins.map((bin, i) =>
        i === binIndex ? { ...bin, count: bin.count + 1 } : bin
      );
    }, initialBins);
  };
                      ...
                      {['single', 'with_children', 'retired'].map(familyStatus => {
                        ...
                        return (
                          <tr key={familyStatus}>
                            ...
                            {['employed', 'unemployed', 'self employed', 'retired'].map(employmentStatus => {
                              ...
                              return (
                                <td key={employmentStatus}>
                                  {item ? (
                                    <>
                                      {item.count} <small className="text-muted">({formatPercentage(item.count, voters.length)})</small>
                                    </>
                                  ) : '0'}
                                </td>
                              );
                            ...
                            <td>
                              {total} <small className="text-muted">({formatPercentage(total, voters.length)})</small>
                            </td>
                          </tr>
                        );
                      })}
