...
const VoterTooltip: React.FC<{ tip: TooltipState; t: (k: string) => string }> = ({ tip, t }) => (
  ...
  >
    {tip.winnerA && (
      <div>
        {t('ideologyMap.tooltipA')} {tip.winnerA}:{' '}
        <strong>{tip.voter.utility_winner_a.toFixed(3)}</strong>
      </div>
    )}
    {tip.winnerB && (
      <div>
        {t('ideologyMap.tooltipB')} {tip.winnerB}:{' '}
        <strong>{tip.voter.utility_winner_b.toFixed(3)}</strong>
      </div>
    )}
  </div>
);
...
const IdeologyMapChart: React.FC<Props> = ({
  ...
  useEffect(() => {
    setCandidatePositions(initCandidates());
  }, [initCandidates]);
  ...
  useDragTouch(svgRef, {
    onStart: (_x, _y) => {
      /* index tracked via handleCandidatePointerDown */
    },
    onMove: (x, y) => {
      if (draggingIdx === null) return;
      setCandidatePositions((prev) => prev.map((c, i) => (i === draggingIdx ? { ...c, x, y } : c)));
    },
    onEnd: () => {
      setDraggingIdx(null);
      setTimeout(() => {
        setCandidatePositions((current) => {
          fetchMap(current);
          return current;
        });
      }, 150);
  ...
  const candidates =
    mapData?.candidates ?? candidatePositions.map((c) => ({ ...c, party: 'Independent' }));
  ...
  return (
    <Row className="g-3">
        ...
        <Col xs={12} lg={3}>
          <Card>
            ...
            <CardBody className="p-2">
              <label className="mb-1 inline-block text-sm mb-1">{t('ideologyMap.methodA')}</label>
              <Select
                size="sm"
                value={methodA}
                className="mb-2"
              ...
              <label className="mb-1 inline-block text-sm mb-1">{t('ideologyMap.methodB')}</label>
              <Select
                size="sm"
                value={methodB}
                className="mb-2"
              ...
              <label className="mb-1 inline-block text-sm mb-1">{t('ideologyMap.ideology')}</label>
              <Select
                size="sm"
                value={ideology}
                className="mb-2"
              ...
              <Check
                ...
                label={<span className="text-sm">{t('ideologyMap.showLosers')}</span>}
                checked={showLosers}
              ...
              <Check
                ...
                label={<span className="text-sm">{t('ideologyMap.showVoronoi')}</span>}
                checked={showVoronoi}
              ...
              <Check
                ...
                label={<span className="text-sm">{t('ideologyMap.showMedian')}</span>}
                checked={showMedian}
              ...
              {/* View mode toggle */}
              <div className="mb-3">
                <label className="mb-1 inline-block text-sm mb-1">{t('heatmap.viewMode')}</label>
              ...
              {loading && (
                <div className="text-center mt-2">
                  ...
                  <span className="text-sm text-muted-foreground">{t('ideologyMap.loading')}</span>
                </div>
              )}
            </CardBody>
          </Card>
      ...
      <Col xs={12} lg={embedded ? 12 : 9}>
        ...
        {embedded && (
          <div
            className="flex gap-1 flex-wrap mb-2 items-center"
            style={{ fontSize: '0.7rem' }}
            data-testid="embedded-layer-bar"
          >
            <span className="text-muted-foreground me-1">{t('heatmap.viewMode')}:</span>
            <ButtonGroup size="sm">
              {(['points', 'heatmap', 'both'] as const).map((mode) => (
                <Button
                  key={mode}
            ...
            <span className="text-muted-foreground ms-2 me-1">|</span>
            <Check
              type="switch"
              id="emb-show-voronoi"
              checked={showVoronoi}
            ...
            {animActive && (
              ...
              >
                ●{' '}
                {t('lab.animLiveOnMap', {
                  method: animFrame?.method ?? '',
                  step: animFrame?.step ?? 1,
                  total: animFrame?.totalSteps ?? 1,
                })}
              </Badge>
            )}
          </div>
        ...
        <Card>
          <CardBody className="p-2">
            {error && <div className="text-[#dc3545] text-sm mb-2">{error}</div>}
            ...
            {(viewMode === 'points' || viewMode === 'both') && (
              ...
              >
                ...
                {/* Axis labels */}
                <text x={MARGIN} y={SVG_H - 8} fontSize={10} fill="#6c757d">
                ...
                <text
                  ...
                  fontSize={10}
                  fill="#6c757d"
                  transform={`rotate(-90 8 ${MARGIN + 4})`}
                >
                  {t('ideologyMap.axisLiberal')}
                </text>
                <text
                  x={8}
                  y={SVG_H - MARGIN}
                  fontSize={10}
                  fill="#6c757d"
                  transform={`rotate(-90 8 ${SVG_H - MARGIN})`}
                >
                  {t('ideologyMap.axisConservative')}
                </text>
                ...
                {voters.map((v) => (
                  <circle
                    ...
                    fill={voterColor(v)}
                    fillOpacity={
                      showLosers && Math.max(v.utility_winner_a, v.utility_winner_b) < 0.3
                        ? 0.8
                        : 0.35
                    }
                    ...
                    >
                      ...
                      {(isWinnerA || isWinnerB) && !isEliminated && (
                        <circle
                          ...
                          strokeWidth={2.5}
                          stroke={
                            isWinnerA && isWinnerB ? '#7B2D8B' : isWinnerA ? COLOR_A : COLOR_B
                          }
            ...
            {mapData && (
              <div className="mt-3">
                ...
                <div className="mb-1" style={{ fontSize: '0.78rem', color: '#6c757d' }}>
                  {pctA}% {t('ideologyMap.prefer')}{' '}
                  <strong style={{ color: COLOR_A }}>{methodA}</strong>
                  {' / '}
                  {pctB}% {t('ideologyMap.prefer')}{' '}
                  <strong style={{ color: COLOR_B }}>{methodB}</strong>
                </div>
                <div className="flex rounded overflow-hidden mb-3" style={{ height: 14 }}>
                  <div
                    style={{ width: `${pctA}%`, background: COLOR_A, transition: 'width 0.4s' }}
                  />
                  <div
                    style={{ width: `${pctB}%`, background: COLOR_B, transition: 'width 0.4s' }}
                  />
                </div>

                {/* Winner badges */}
                <div className="flex gap-2 flex-wrap mb-2">
                  <span className="text-sm text-muted-foreground">{t('ideologyMap.winners')}</span>
                  {winnerA && (
                  ...
                  {condorcet && <Badge variant="secondary">Condorcet: {condorcet}</Badge>}
                </div>
