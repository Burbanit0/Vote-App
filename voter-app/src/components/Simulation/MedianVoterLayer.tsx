...
export interface MedianVoterPoint { x: number; y: number }
export interface MedianCandidate  { name: string; x: number; y: number }
...
export function nearestCandidate(
  mx: number, my: number, candidates: MedianCandidate[],
    ...
    <g data-testid="median-voter-layer" pointerEvents="none">
      ...
      {winnerA && (() => {
        ...
        return (
          <line
            x1={svgMx} y1={svgMy}
            x2={domainToSvgX(wa.x)} y2={domainToSvgY(wa.y)}
            stroke={colorA}
            strokeWidth={2}
      ...
      {winnerB && winnerB !== winnerA && (() => {
        ...
        return (
          <line
            x1={svgMx} y1={svgMy}
            x2={domainToSvgX(wb.x)} y2={domainToSvgY(wb.y)}
            stroke={colorB}
            strokeWidth={2}
      ...
      {/* ── Pulsing halo ── */}
      <circle cx={svgMx} cy={svgMy} r={16}
        fill={MEDIAN_COLOR} fillOpacity={0.08}
        stroke={MEDIAN_COLOR} strokeWidth={1} strokeOpacity={0.3}
      ...
      <g data-testid="median-cross">
        <line
          x1={svgMx - crossSize} y1={svgMy}
          x2={svgMx + crossSize} y2={svgMy}
          stroke={MEDIAN_COLOR} strokeWidth={2.5}
        />
        <line
          x1={svgMx} y1={svgMy - crossSize}
          x2={svgMx} y2={svgMy + crossSize}
          stroke={MEDIAN_COLOR} strokeWidth={2.5}
        />
      </g>
      ...
      <g transform={`translate(${svgPx}, ${svgPy - 30})`}>
        <rect x={-45} y={-10} width={90} height={18} rx={4}
          fill={MEDIAN_COLOR} fillOpacity={0.12}
          stroke={MEDIAN_COLOR} strokeWidth={0.8}
        />
  ...
  return (
    ...
    >
      <div className="d-flex flex-wrap gap-3">
        <span>
          <span style={{ color: MEDIAN_COLOR, fontWeight: 600 }}>✛ {t('ideologyMap.medianVoter')}</span>
          {' '} x={mx.toFixed(2)}
        </span>
        <span>
          <strong>{t('ideologyMap.medianPredicts')}: {predicted.name}</strong>
        </span>
        {winnerA && (
          <span style={{ color: matchA ? HIT_COLOR : MISS_COLOR }}>
            {methodA}: {winnerA} {matchA ? '✓' : `✗ (±${Math.abs(
              (candidates.find(c => c.name === winnerA)?.x ?? 0) - mx
            ).toFixed(2)})`}
          </span>
        )}
        {winnerB && winnerB !== winnerA && (
