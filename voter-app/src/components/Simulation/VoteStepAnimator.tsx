  ...
  if (round.winner) {
    return (
      <div className="text-center py-4">
        <div style={{ fontSize: '1.4rem', fontWeight: 700, color: C.green }}>
          🏆 {round.winner}
        </div>
        <div className="text-muted small mt-1">{t('animation.winner')}</div>
      </div>
    );
  }
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={data} layout="vertical" margin={{ left: 10, right: 40, top: 4, bottom: 4 }}>
          ...
          <XAxis type="number" domain={[0, 100]} unit="%" tick={{ fontSize: 10, fill: ct.tickFill }} />
          <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: ct.tickFill }} width={60} />
          ...
          <Bar dataKey="pct" radius={[0, 4, 4, 0]} isAnimationActive animationDuration={400}>
            <LabelList dataKey="pct" position="right" style={{ fontSize: 11, fill: ct.tickFill }}
              formatter={(v) => `${v}%`} />
            {data.map((entry) => (
              <Cell key={entry.name} fill={candidateColor(entry.name, allCandidates, round.eliminated)} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
  ...
  return (
    <div>
      <div className="text-muted small mb-1">
        ...
        <strong> {step.points_awarded} {t('animation.borda_pts')}</strong>
      </div>
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={data} margin={{ top: 16, right: 20, bottom: 20, left: 10 }}>
          ...
          <Bar dataKey="points" radius={[4, 4, 0, 0]} isAnimationActive animationDuration={400}>
            <LabelList dataKey="points" position="top" style={{ fontSize: 10, fill: ct.tickFill }} />
            {data.map((entry) => (
              <Cell
                key={entry.name}
                fill={isLast && entry.name === winner ? C.green : candidateColor(entry.name, allCandidates)}
              />
            ))}
          </Bar>
        </BarChart>
...
const PluralityChart: React.FC<{
  ...
  const data = allCandidates.map((name) => ({
    name, pct: Math.round((firstChoices[name] ?? 0) * 100),
  }));
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={data} layout="vertical" margin={{ left: 10, right: 50, top: 4, bottom: 4 }}>
          ...
          <Bar dataKey="pct" radius={[0, 4, 4, 0]} isAnimationActive animationDuration={600}>
            <LabelList dataKey="pct" position="right" style={{ fontSize: 11 }}
              formatter={(v) => `${v}%`} />
            {data.map((entry) => (
              <Cell key={entry.name}
...
const SchulzeHeatmap: React.FC<{
  ...
  const cell = (v: number) => {
    ...
    const bg = v > 0.5 ? `rgba(0,122,51,${0.15 + v * 0.5})` : `rgba(183,28,28,${0.1 + (1 - v) * 0.35})`;
  ...
  return (
    <div>
      <div className="text-muted small mb-1 fw-semibold">{title}</div>
        ...
        <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
          <thead>
            <tr>
              ...
              {candidates.map((c) => (
                <th key={c} style={{ width: CELL_W, padding: '3px 6px', textAlign: 'center',
          ...
          <tbody>
            {candidates.map((row) => (
              <tr key={row}>
                ...
                {candidates.map((col) => {
                  if (row === col) return (
                    <td key={col} style={{ padding: '3px 6px', textAlign: 'center',
                      background: '#f0f0f0', color: '#999' }}>—</td>
                  );
                  const { bg, text } = cell(matrix[row]?.[col] ?? 0);
                  return (
                    <td key={col} title={`${row} vs ${col}`}
                      style={{ padding: '3px 6px', textAlign: 'center', background: bg,
...
const ApprovalChart: React.FC<{
  ...
  const data = allCandidates.map((name) => ({
    name, pct: Math.round((scores[name] ?? 0) * 100),
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={data} layout="vertical" margin={{ left: 10, right: 50, top: 4, bottom: 4 }}>
          ...
          <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: ct.tickFill }} width={60} />
          <Tooltip contentStyle={ct.tooltipStyle} formatter={(v: number) => [`${v}%`, t('animation.approval_rate')]} />
...
const VoteStepAnimator: React.FC<Props> = ({ defaultCandidates }) => {
  ...
  const totalSteps = (() => {
    ...
    switch (stepData.method) {
      case 'irv':      return stepData.rounds.length;
      case 'borda':    return stepData.steps.length;
      case 'plurality': return 1;
      case 'schulze':  return 2;   // duel → path
  ...
  const handleNext    = () => { setCurrentStep((s) => Math.min(s + 1, totalSteps - 1)); setPlaying(false); };
  const handleRestart = () => { setCurrentStep(0); setPlaying(false); setApprovalAnimated(false); };
  const handlePlay    = () => {
  ...
  const renderStep = () => {
    ...
    if (stepData.method === 'borda') {
      ...
      return (
        <BordaChart
          step={step} allCandidates={candidates}
          winner={stepData.winner} isLast={currentStep === stepData.steps.length - 1}
          ct={ct} t={t}
        />
      );
    }
    ...
    if (stepData.method === 'plurality') {
      return (
        <PluralityChart
          firstChoices={stepData.first_choices} winner={stepData.winner}
          allCandidates={candidates} ct={ct} t={t}
        />
      );
    }
    ...
    if (stepData.method === 'approval') {
      return (
        <ApprovalChart
          scores={stepData.approval_scores} threshold={stepData.threshold_used}
          winner={stepData.winner} allCandidates={candidates}
          animated={approvalAnimated} ct={ct} t={t}
        />
      );
    }
  ...
  const stepLabel = (() => {
    if (!stepData) return '';
    if (stepData.method === 'irv')     return `${t('animation.round')} ${currentStep + 1} / ${totalSteps}`;
    if (stepData.method === 'borda')   return `${t('animation.step')} ${currentStep + 1} / ${totalSteps}`;
    if (stepData.method === 'schulze') return currentStep === 0 ? t('animation.schulze_duels') : t('animation.schulze_paths');
    return '';
  })();
    ...
    <Card>
      <Card.Header className="d-flex align-items-center justify-content-between flex-wrap gap-2 py-2">
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          {/* Controls */}
          <ButtonGroup size="sm">
            <Button variant={playing ? 'primary' : 'outline-primary'} onClick={handlePlay}
      ...
      <Card.Body style={{ minHeight: 220 }}>
        {loading && (
          <div className="d-flex align-items-center justify-content-center py-5">
            ...
            <span className="text-muted small">{t('animation.loading')}</span>
          </div>
        )}
