...
const InformationModelPanel: React.FC<Props> = ({ candidateNames, config, onChange, result }) => {
  ...
  const handleSegment = useCallback(
    (key: 'low_info' | 'medium_info' | 'high_info', rawValue: number) => {
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          <Check
            ...
            label={
              <span className="font-semibold">Activer le modèle d'information asymétrique</span>
            }
            checked={enabled}
            onChange={handleToggle}
          />
          <div className="text-muted-foreground text-sm mt-1">
            Les électeurs perçoivent les positions des candidats avec un biais médiatique et un
            niveau de bruit dépendant de leur exposition à l'information.
          </div>
        </div>
      </div>
          ...
          <Card className="mb-3">
            ...
            <CardBody>
              ...
              {(
                ...
                <Row key={key} className="items-center mb-2 g-2">
                  <Col xs={5}>
                    <OverlayTrigger
                      placement="right"
                      overlay={
          ...
          <Card className="mb-3">
            ...
            <CardBody>
              ...
              {candidateNames.map((name, idx) => {
                ...
                return (
                  <Row key={idx} className="items-center mb-2 g-2">
                    <Col xs={4}>
                      <span className="font-semibold" style={{ fontSize: '0.88rem' }}>
                        {name}
                      </span>
                    </Col>
                    <Col xs={5}>
                    ...
                    <Col xs={3} className="flex items-center gap-1">
                      <span
                        className="font-semibold"
                        style={{ color: biasColor(bias), fontSize: '0.82rem' }}
                      >
                        {bias >= 0 ? '+' : ''}
                        {bias.toFixed(2)}
                      </span>
              ...
              <CardBody>
                <Row className="g-3 mb-3">
                  <Col md={6}>
                    <div className="text-muted-foreground text-sm mb-1">
                      Gagnant sincère (vraies utilités)
                    </div>
                    <Badge
                  ...
                  <Col md={6}>
                    <div className="text-muted-foreground text-sm mb-1">
                      Gagnant perçu (utilités biaisées)
                    </div>
                    <Badge
                      style={{
                        fontSize: '0.9rem',
                ...
                {result.winners_differ && (
                  <Alert variant="danger" className="py-2 mb-3" style={{ fontSize: '0.88rem' }}>
                    ⚠️ <strong>Les biais d'information changent le résultat !</strong> En réalité,{' '}
                    <strong>{result.sincere_winner}</strong> aurait gagné. La désinformation conduit
                    à l'élection de <strong>{result.perceived_winner}</strong>.
                  </Alert>
                )}
                ...
                <div className="flex gap-4 flex-wrap" style={{ fontSize: '0.85rem' }}>
                  <div>
                    <span className="text-muted-foreground">Écart d'information :</span>{' '}
                    <strong style={{ color: gapColor(result.information_gap ?? 0) }}>
                      {((result.information_gap ?? 0) * 100).toFixed(1)} %
                    </strong>
                    <span className="text-muted-foreground ms-1" style={{ fontSize: '0.75rem' }}>
                  ...
                  {result.sincere_condorcet_winner && (
                    <div>
                      <span className="text-muted-foreground">
                        Vainqueur de Condorcet sincère :
                      </span>{' '}
