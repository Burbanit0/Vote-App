...
const EvaluativeChart: React.FC<EvaluativeChartProps> = ({ evResult, candidates }) => {
  ...
  const totalVoters =
    Object.values(evResult.ev_distribution[sorted[0]] ?? {}).reduce((s, v) => s + v, 0) || 1;
  ...
  return (
    <div data-testid="evaluative-chart">
        ...
        <div className="flex items-center gap-2 mb-2">
          <Badge variant="success" style={{ fontSize: '0.78rem' }} data-testid="ev-winner-badge">
            ...
            <span style={{ fontWeight: 400 }}>
              ({evResult.scores[evResult.winner] >= 0 ? '+' : ''}
              {evResult.scores[evResult.winner]})
            </span>
          </Badge>
        </div>
      )}
      ...
      >
        {/* Column headers */}
        <text x={LABEL_W + BAR_W / 2} y={14} textAnchor="middle" fontSize={8} fill="#6c757d">
          {t('ev.axisLabel')}
        </text>
        ...
        {sorted.map((cand, ri) => {
          ...
          return (
            <g key={cand} data-testid={`ev-row-${cand}`}>
              ...
              {/* Track background */}
              <rect x={LABEL_W} y={y + 6} width={BAR_W} height={ROW_H - 12} rx={3} fill="#f8f9fa" />
              ...
              {approveW > 0 && (
                ...
                >
                  <title>
                    {cand}: +{dist['+1']} approvals ({Math.round(pctApprove * 100)}%)
                  </title>
                </rect>
              )}
              ...
              {rejectW > 0 && (
                ...
                >
                  <title>
                    {cand}: −{dist['-1']} rejections ({Math.round(pctReject * 100)}%)
                  </title>
                </rect>
              )}
