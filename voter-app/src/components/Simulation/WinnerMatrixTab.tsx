...
import AnimatedVoteCount, { generateDemoBallots, VoteMethod } from '../pedagogy/AnimatedVoteCount';
...
const WinnerMatrixTable: React.FC<TableProps> = ({
  ...
  return (
    <ResponsiveTable>
      {label && (
        <p className="font-semibold text-center mb-2" style={{ fontSize: '0.9rem' }}>
          {label}
        </p>
      )}
      <Table
...
const WinnerMatrixTab: React.FC<Props> = ({
  ...
  const legend = (
    <div className="mb-3 flex gap-3 flex-wrap items-center">
      ...
      ) && <small className="text-muted-foreground">{t('simulation.notCondorcet')}</small>}
    </div>
  );
    ...
    <>
            ...
            <Modal show centered size="lg" onHide={() => setDrilldownTarget(null)}>
              <Modal.Header closeButton>
                <Modal.Title>
                  ...
                  <span
                    className="text-muted-foreground font-normal ms-2"
                    style={{ fontSize: '1rem' }}
                  >
              ...
              <Modal.Body>
                <div className="text-center mb-4">
                  <div className="text-muted-foreground text-sm mb-1">{t('simulation.winner')}</div>
                  {winner ? (
                    <Badge
                      style={{
                    ...
                    <span className="text-muted-foreground">{t('simulation.noWinner')}</span>
                  )}
                </div>
                ...
                <Table className="[&_th]:p-1 [&_td]:p-1 [&_th]:text-left [&_td]:border-t [&_th]:border-b [&_td]:border-border [&_th]:border-border [&_*]:align-middle [&_th]:border [&_td]:border mb-3">
                  <tbody>
                    {[
                      {
                        label: t('simulation.bayesianRegret'),
                        value: m.bayesian_regret != null ? m.bayesian_regret.toFixed(4) : '—',
                        note: t('simulation.lowerBetter'),
                      },
                      {
                        label: t('simulation.majoritySatisfaction'),
                        value:
                          m.majority_satisfaction != null
                            ? `${(m.majority_satisfaction * 100).toFixed(1)} %`
                            : '—',
                        note: t('simulation.higherBetter'),
                      },
                      {
                        label: t('simulation.strategicVulnerability'),
                        value:
                      ...
                      <tr key={label}>
                        <td className="font-semibold" style={{ width: '40%' }}>
                          {label}
                        </td>
                        <td>{value}</td>
                ...
                <p className="text-muted-foreground text-sm mb-0" style={{ lineHeight: 1.6 }}>
                  <strong>
                    <MethodTooltip method={methodKey} />:{' '}
                  </strong>
      ...
      {animateMethod && animProps && (
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              🎬 Décompte animé — <MethodTooltip method={animateMethod} />
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <AnimatedVoteCount
            ...
            {!expertMode && (
              <Alert variant="info" className="mt-3 py-2 mb-0" style={{ fontSize: '0.82rem' }}>
                ...
                représentatifs générés pour illustrer le fonctionnement de la méthode. Les vrais
                bulletins sont calculés côté serveur avec un modèle d'utilité spatial.
              </Alert>
            )}
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={() => setAnimateMethod(null)}>
              {t('common.close')}
            </Button>
          </Modal.Footer>
        </Modal>
