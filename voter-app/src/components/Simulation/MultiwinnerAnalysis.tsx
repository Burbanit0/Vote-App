...
const MultiwinnerAnalysis: React.FC = () => {
  ...
  const addParty = () =>
    setParties((prev) => [
      ...prev,
      { name: `Party ${String.fromCharCode(65 + prev.length)}`, votes: '5' },
    ]);
    ...
    <div>
      ...
      {result && (
        <>
          ...
          <div className="flex gap-3 mb-4 flex-wrap">
            {result.comparison.most_proportional && (
              <Alert variant="success" className="py-2 mb-0 grow">
                ...
                {METHOD_LABELS[result.comparison.most_proportional]} (Gallagher{' '}
                {(result as any)[
                  result.comparison.most_proportional
                ].metrics.gallagher_index?.toFixed(3)}
                )
              </Alert>
            )}
            {result.comparison.least_proportional && (
              <Alert variant="warning" className="py-2 mb-0 grow">
                <strong>Least proportional:</strong>{' '}
                {METHOD_LABELS[result.comparison.least_proportional]} (Gallagher{' '}
                {(result as any)[
                  result.comparison.least_proportional
                ].metrics.gallagher_index?.toFixed(3)}
                )
          ...
          <Card className="mb-4">
            <CardHeader className="block space-y-0 border-b border-border px-4 py-2">
              <strong>Proportionality Metrics</strong>
            </CardHeader>
                ...
                <Table className="[&_th]:p-1 [&_td]:p-1 [&_th]:text-left [&_td]:border-t [&_th]:border-b [&_td]:border-border [&_th]:border-border [&_*]:align-middle [&_th]:border [&_td]:border mb-0">
                  <thead className="table-light">
                    <tr>
                      ...
                      <th
                        ...
                        title="Gallagher LSq index — 0 = perfectly proportional"
                      >
                        Gallagher ↓
                      </th>
                      <th
                        className="text-center"
                        title="Effective number of parties in seats (Laakso-Taagepera)"
                      >
                        ENP seats
                      </th>
                  ...
                  <tbody>
                    {result.comparison.gallagher_ranking.map((method, rank) => {
                      const m = (result as any)[method] as {
                      ...
                      return (
                        <tr
                          key={method}
                          style={isBest ? { backgroundColor: '#f0fff4' } : undefined}
                        >
                          <td className="font-semibold ps-2">
          ...
          <Card>
            <CardHeader className="block space-y-0 border-b border-border px-4 py-2">
              <strong>Seat Breakdown Detail</strong>
            </CardHeader>
            <CardBody className="p-0">
              <ResponsiveTable>
                <Table className="[&_th]:p-1 [&_td]:p-1 [&_th]:text-left [&_td]:border-t [&_th]:border-b [&_td]:border-border [&_th]:border-border [&_*]:align-middle [&_th]:border [&_td]:border mb-0 text-center">
                  <thead className="table-light">
                    <tr>
                      <th className="text-left ps-2">Party</th>
                      <th>Votes</th>
                      <th>Ideal</th>
                  ...
                  <tbody>
                      ...
                      return (
                        <tr key={p}>
                          ...
                          <td className="text-muted-foreground">
                            {pct(result.party_votes[p], totalVotes)}
                          </td>
                          <td className="text-muted-foreground">{idealSeats}</td>
                          {PROPORTIONAL_METHODS.map((method) => {
                    ...
                    <tr className="table-light font-semibold">
                      <td className="text-left ps-2">Total</td>
                      <td>100%</td>
                      <td>{result.num_seats}</td>
                      {PROPORTIONAL_METHODS.map((method) => (
                        <td key={method}>
                          {Object.values(
                            (result as any)[method].seats as Record<string, number>
                          ).reduce((a, b) => a + b, 0)}
                        </td>
                      ))}
                    </tr>
