...
const MechChart: React.FC<{
  ...
  const chartData = names.map((name) => ({
    ...
    matching: Math.max(
      0,
      (alloc[name] ?? 0) - (data.projects.find((p) => p.name === name)?.private_funding ?? 0)
    ),
  }));
  const gini = data.gini_coefficients[mechanism] ?? 0;
  const winner = names.reduce(
    (best, n) => ((alloc[n] ?? 0) > (alloc[best] ?? 0) ? n : best),
    names[0]
  );

  return (
    <div data-testid={`mech-chart-${mechanism}`}>
      <div className="text-center font-semibold mb-1" style={{ fontSize: '0.8rem' }}>
      ...
      <div style={{ height: 180 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 20, left: -10 }}>
            ...
            />
            <Bar dataKey="private" stackId="a" name="private" isAnimationActive={false}>
              {chartData.map((entry, i) => (
                <Cell key={i} fill={projColor(entry.fullName, names)} fillOpacity={0.55} />
              ))}
            </Bar>
            <Bar dataKey="matching" stackId="a" name="matching" isAnimationActive={false}>
              {chartData.map((entry, i) => (
      ...
      <div className="flex justify-between mt-1" style={{ fontSize: '0.7rem' }}>
        <Badge
          variant="secondary"
          data-testid={`gini-badge-${mechanism}`}
  ...
  return (
    <Container className="py-4" style={{ maxWidth: 1100 }}>
      <div className="mb-4">
        ...
        <p className="text-muted-foreground" style={{ fontSize: '0.9rem', maxWidth: 700 }}>
          {t('qf.pageSubtitle')}
        </p>
      </div>
      ...
      <Row className="g-4">
        ...
        <Col xs={12} md={4}>
          <Card>
            ...
            <CardBody className="p-3">
              {/* Projects */}
              <div className="mb-3">
                <div className="font-semibold mb-2" style={{ fontSize: '0.8rem' }}>
                  {t('qf.projects')}
                </div>
                {projects.map((p, i) => (
                  <div
                    ...
                    style={{
                      borderLeft: `3px solid ${projColor(p.name, projNames)}`,
                      fontSize: '0.78rem',
                    }}
                  >
              ...
              <Range
                ...
                step={10}
                value={budgetPV}
                onChange={(e) => setBudgetPV(Number(e.target.value))}
                className="mb-2"
              />

              <label className="mb-1 inline-block text-sm mb-1">{t('qf.ideology')}</label>
              <Select
                size="sm"
                value={ideology}
              ...
              {error && (
                <Alert variant="danger" className="mt-2 py-2" style={{ fontSize: '0.8rem' }}>
                  {error}
                </Alert>
              )}
            </CardBody>
        ...
        {/* Right: results */}
        <Col xs={12} md={8}>
          {!data && !loading && <Alert variant="info">{t('qf.prompt')}</Alert>}
          ...
          {data && (
            <>
              ...
              <div className="border border-border rounded p-3 mb-3">
                <label
                  className="mb-1 inline-block font-semibold mb-0"
                  ...
                  <span key={n} className="flex items-center gap-1" style={{ fontSize: '0.72rem' }}>
                    <span
                      style={{
                        width: 10,
                        height: 10,
                        background: projColor(n, projNames),
                        borderRadius: 2,
                        display: 'inline-block',
                      }}
                    />
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {(['1p1v', 'proportional', 'qf'] as const).map((m) => (
                  <Badge
                    key={m}
                    variant={
                      data.gini_coefficients[m] < 0.2
                        ? 'success'
                        : data.gini_coefficients[m] < 0.4
                          ? 'warning'
                          : 'danger'
                    }
              ...
              >
                <div className="font-semibold mb-1">💡 {t('qf.insightTitle')}</div>
