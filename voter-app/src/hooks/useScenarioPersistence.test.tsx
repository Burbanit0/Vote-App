...
function Harness() {
  ...
  return (
    <div>
      ...
      <button data-testid="open-save" onClick={p.openSave}>
        open save
      </button>
      <button data-testid="close-save" onClick={p.closeSave}>
        close save
      </button>
      <button data-testid="set-name" onClick={() => p.setName('My scenario')}>
        set name
      </button>
      <button
        data-testid="save"
        onClick={() => {
          p.saveCurrent({ a: 1 }, { b: 2 }).catch(() => {});
        }}
      >
        save
      </button>
      <button data-testid="open-load" onClick={() => p.openLoad()}>
        open load
      </button>
      <button data-testid="close-load" onClick={p.closeLoad}>
        close load
      </button>
      <button data-testid="remove" onClick={() => p.removeFromList(7)}>
        remove 7
      </button>
    </div>
  );
...
describe('useScenarioPersistence', () => {
  ...
  it('openSave shows the modal and clears the previous name', () => {
    ...
    act(() => {
      screen.getByTestId('open-save').click();
    });
  ...
  it('openLoad fetches the list and stores it', async () => {
    ...
    render(<Harness />);
    await act(async () => {
      screen.getByTestId('open-load').click();
    });
  ...
  it('closeLoad just hides the modal (does not touch the list)', async () => {
    ...
    act(() => {
      screen.getByTestId('close-load').click();
    });
    expect(screen.getByTestId('show-load')).toHaveTextContent('no');
    expect(screen.getByTestId('list')).toHaveTextContent('1');
  });
