...
| **Merge queue** | Résout structurellement l'invalidation en cascade (« doit être à jour avec develop ») : la queue rebase et teste en série toute seule. | S | ⭐⭐⭐ | 📝📝 | 🔶 native GitHub **bloquée** (réservée aux repos publics *organisation*, pas compte perso — confirmé par un 422 vide sur l'API rulesets) ; pivot vers Mergify (gratuit OSS, `required_status_checks` déjà injecté automatiquement). **Effet de bord utile** : drift réel trouvé et corrigé — `develop` n'avait que 5 checks requis sur les 12 documentés (`scripts/setup-branch-protection.sh`), restauré en direct. Reste : installer l'app Mergify (action utilisateur), puis désactiver `strict` (« up to date ») une fois Mergify vérifié en marche — Mergify le documente lui-même comme incompatible avec ses checks parallèles |
