...
2. **Seuil de candidature indépendante** : un citoyen hors parti ne devient
   candidat que s'il franchit un seuil de soutien préalable (parrainages
   simulés) — mécanique d'accès au bulletin standard dans les démocraties
   réelles, qui évite un scrutin à 200 candidats sans limite artificielle.
...
`0.01` pour un indépendant classique), pas d'une exemption totale. Sans
ça, le mécanisme de bornage du §2.3 était purement et simplement annulé.
...
#### 3.6.6 `pressure_action` — la décision citoyenne de pression 🟢 *(rév. 2b)*
...
**🔴 Ouvert** : graphe social statique ou évolutif (homophilie) ?
...
**🔴 Ouvert** : pouvoir réel de la chambre — veto suspensif limité
...
- Modèles open-weight pour démarrer (Qwen3, Gemma, Mistral Small), Ollama
  ...
  validée.
- Fine-tuning par distillation théorique, en **toute fin** de feuille de
  route (v8).
...
| A6 | Formule de `L(t)` | 🟡 | `écart(t)` défini (§7bis.0) ; `support(t)` reste à définir |
...
| B4 | Budget de coût | 🟡 | Reformulé en **temps d'horloge** et non en prix (§15bis.0) ; ordres de grandeur posés, mesure réelle à faire sur un run de 100 citoyens |
...
**Reste bloquant avant v2** : B3 (critère de cohorte) et B4 (chiffrage).
**Reste bloquant avant v4** : `support(t)` de A6.
...
4. Scission de parti : dans le scope ou non ?
5. Régénération des personas : à quels points de rupture précis ?
6. ~~Fréquence des snapshots~~ — **tranché** : annuelle, 4 ticks (§16.4).
7. ~~Granularité des justifications LLM~~ — **tranché** : codes partout +
   texte libre borné aux pivots (§16.5).
8. Stockage (§16.6) : DuckDB par run, ou PostgreSQL ?
9. Format du vote de confiance (§7bis.4a) : binaire, ou avec challenger ?
10. Calibration de `w_pet` / `w_mob` dans `écart(t)` (§7bis.6).
...
13. Distribution des `base_threshold` sur les personas (§7bis.9c) — c'est
    ...
    et le plus sensible au coût LLM.
14. Codebook (§3.7) injecté en entier dans chaque prompt, ou porté par le
    ...
    réel — à vérifier avant de trancher (§15).
15. Faut-il coder `event_type` du journal en entier (§16.3), par
    cohérence avec le codebook ?
...
20. Le déterminisme de vLLM sous batching variable (§15bis.4c) tient-il ?
    **Question ouverte la plus critique du plan** : elle conditionne le
    test de reproductibilité octet-pour-octet, lui-même garde-fou de tout
    le projet. Protocole de vérification en §15bis.5.
21. Sharding intra-run (§15bis.3) : à n'envisager qu'avec plusieurs GPU
    physiques, et seulement après validation du point 20.
