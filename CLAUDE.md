...
- **Frontend**: React 19 + TypeScript · React Router v7 · **Tailwind v4 + shadcn/ui** (`src/components/ui/`; Bootstrap/react-bootstrap fully removed) · Recharts · D3 7.x · i18next (FR/EN) · Vite · PWA (vite-plugin-pwa)
  - **UI layer** (Phase 6): hand-written shadcn-style primitives in `src/components/ui/` (button/badge/alert/card/grid/form-controls/table/modal/bootstrap-tabs/accordion/dropdown/toast/pagination/button-group/navbar/tooltip-overlay/…). `tailwind.css` uses full `@import "tailwindcss"` (preflight on). Grid (`Container/Row/Col`) reimplements Bootstrap's 12-col flex grid; legacy `var(--bs-*)` inline styles still resolve via vars defined in `tailwind.css`.
...
Phase 5.4) ; `useElection()` (réexporté par le store) reste l'API des consommateurs.
Contenu :
...
- **Stores Zustand**: les hooks de convenance sélectionnent les champs **un par un** (`useStore(s => s.x)`) — ne jamais retourner un objet composite frais depuis un seul sélecteur (casse le cache de snapshot). Les stores s'auto-hydratent du localStorage à l'init (pas de Provider) ; `hydrate()` existe pour l'isolation des tests (`useXStore.setState(reset)` en `beforeEach`).
