"""
scripts/check_vllm_coalition_decision_collapse_signature.py

Phase 1 of plan-flagship-30y-run.md: this decision type's own RELIABILITY WARNING
(byte-identical decisions across structurally opposite input poles) was measured on Ollama
only. The flagship run targets vLLM/AWQ, a different serving stack and quantization -- the
truncation bug already proved backend-specific behaviour is real for this project, so
re-running this exact protocol, unmodified except for the client, is what actually answers
whether the warning still applies before spending days of GPU on a run that assumes it does
or doesn't. Same poles, same citizens/parties, same schema, same think=False (confirmed
the production call site's own choice, unchanged from the Ollama version) -- only the
client class differs, so a different verdict here is attributable to the backend, not a
changed test.
...
Usage:
    python fast_api_voter/scripts/check_vllm_coalition_decision_collapse_signature.py
"""
...
from api.domain.polity.llm_client import VllmJsonClient, decode_coalition_batch  # noqa: E402
