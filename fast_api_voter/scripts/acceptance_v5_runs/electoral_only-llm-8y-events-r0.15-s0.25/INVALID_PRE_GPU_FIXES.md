...
This run (v5 Lot 5's own acceptance run, cited in `THEORY.md` §10.7 and
`scripts/acceptance_v5_results.md`) completed 2026-08-15, **before** every
one of the reliability fixes this repo's own bug 1-4 investigation
...
- `ca02344` (2026-08-22) — `vote_cast`'s own deterministic, per-voter
  ...
  resampling past it — silent, not a crash) and `chamber_deliberation`'s
  own chunk-size truncation (not applicable to this run — v5 predates dt=11
  entirely — listed for completeness of the investigation timeline only).
...
(3400 events, no crash). Marked because the risk this investigation
identified (particularly the `vote_cast` blank/ranking incoherence) is
**silent by construction**: a run produced before the fix can complete
cleanly, with a schema-valid journal, while still containing individual
decisions this project's own later work found were sometimes wrong.
