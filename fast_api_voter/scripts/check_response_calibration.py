"""
scripts/check_response_calibration.py

lets-build-a-solid-spicy-otter.md Track B1: does build_response_system_
prompt_calibrated (llm_behavior_engine.py, 2026-09-11) restore sensitivity
to representative_response (dt=6)?

check_logprob_response_stance_tracking_results.md, the UNCALIBRATED
baseline, measured P(stance=1)=1.000000, flat to five decimal places,
across the entire crisis<->no-problem spectrum (L 0.95->0.05, mandate_dev
0.0->0.8, street 0.0->3.0) via the SAME nine interpolated points this
script uses. That result is polity-decision-contracts.md's C3 signature --
a decision-relevant quantity sent with no scale the model can calibrate
against -- and it is the exact defect Track B1 fixes: mandate_dev's own
[0,1] geometric bound and street's own decay-derived asymptote, both
stated for the first time in the system prompt.

This script is that same probe, unchanged except for ONE line: the system
prompt builder. Same two pre-registered poles, same 9 interpolation
points, same holders, same seed, same model -- the only variable that
moved is whether the model was ever told what mandate_dev/street's own
numbers mean. A comparison against anything else would not isolate C3.

Real production shape throughout except that one substitution:
build_response_system_prompt_calibrated/build_response_user_prompt,
RESPONSE_JSON_SCHEMA, think=False (this decision type's own production
value), and -- like the original diagnostic -- every call is SOLO
(size=1): decide_representative_response deliberately never batches
(0-or-1 sitting president), so there is no separate batching control to
run here the way pressure_action needed one, and Phase D (cost) is
correspondingly out of scope for this type entirely -- see B1's own
docstring in the plan for why.
...
def main() -> int:
    ...
    with VllmJsonClient.from_config(config.llm, seed=config.run.seed) as client:
        for i in range(_STEPS):
            ...
            system_prompt = build_response_system_prompt_calibrated([holder], config)
