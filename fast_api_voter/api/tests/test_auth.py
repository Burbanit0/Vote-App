...
def _bcrypt_hash(pw: str) -> str:
    """Legacy-style bcrypt hash ($2b$…), as the old Flask-Bcrypt path produced."""
    return bcrypt.hashpw(pw.encode("utf-8"), bcrypt.gensalt()).decode("utf-8")


@pytest.fixture
def existing_user(seed_user):
    """A user created with a legacy bcrypt hash, so we can verify fastapi-users'
    login path reads existing password hashes."""
    uid = seed_user(
        username="legacy",
        email="legacy@example.com",
        hashed_password=_bcrypt_hash("legacypw"),
    )
