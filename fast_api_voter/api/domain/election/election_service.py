"""
election_service.py — the unified election simulation pipeline.

This is the business logic that used to live inside the route itself.
Extracting it:
  ...
  - Decouples the simulation from Flask (no `request`, no `jsonify`, no
    ...
    REST v2, etc.).
  - Makes the route file a thin HTTP adapter: parse JSON → call this
    module → format response.

Contract:
    simulate(data: dict) -> tuple[dict, int]
        data:   the parsed request body (or any dict with the same shape)
        return: (response_body, http_status)

It was an `ElectionService` class holding a single `@staticmethod` — a
namespace, not a type, since it had no state, no other members and no second
implementation. A module is already Python's namespace.

This function stays pure. The Redis memoisation lives one layer up, on the
`api.domain.election.simulate` the routes import, so a direct caller — a test,
a CLI, a batch sweep — gets the real computation. `test_seeded_rng_isolation`
depends on that: it calls simulate twice with identical input and would be
comparing a cached body to itself otherwise.

Errors that are user-facing (e.g. "At least 2 candidates required") return
a 400 status with an "error" key. Internal exceptions propagate to the caller;
on the HTTP path `run_typed` (api/core/worker_dispatch.py) and main.py's
catch-all turn them into 5xx.
"""
